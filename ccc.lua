    ...
    local _ao_ok, _ao_err = pcall(function()
        ...
        local _K = {
            INS_BASE = 2000000000, PKG_SLOT = 3, 
            MELEE_ID = 108,
            MELEE_LO = 108000,
            MELEE_HI = 108999,
...
local function normalizeWeaponLookupID(wid)
    ...
    return wid
        ...
        local function equipWeaponSkin(weaponID, insID)
            ...
            -- [FIX] 近战 ID 规范化 (108001 → 108)，保证缓存 key 一致
            if weaponID >= _K.MELEE_LO and weaponID <= _K.MELEE_HI then
                weaponID = _K.MELEE_ID
            end
            local resID = R.insToRes[insID]
            saveEquip(resID, insID)
    ...
    if currentGunId <= 0 and maxIt <= 0 then return 0 end
        ...
        local function applyWeaponSkinDirect(wpn)
            ...
            if not slua.isValid(wac) then
                pcall(function() if wpn.GetWeaponAvatarComponent then wac = wpn:GetWeaponAvatarComponent() end end)
            end
        ...
        function _G.equip_weapon_avatar(uCharacter)
            ...
            -- 遍历所有库存武器（包含 1-3 号位、近战、手枪）
            local list = nil
            pcall(function()
                if wm.GetAllInventoryWeaponList then
                    list = wm:GetAllInventoryWeaponList(false)
                end
            end)
            if list and slua.isValid(list) then
                local n = 0
                pcall(function() n = list:Num() end)
                for i = 0, n - 1 do
                    local wpn = nil
                    pcall(function() wpn = list:Get(i) end)
        ...
        local function onWeaponLuaInit(_, _, weapon)
            ...
            -- 多次延迟重试，覆盖 mesh 尚未创建的窗口期
            pcall(function()
                if char.AddGameTimer then
                    local delays = { 0.15, 0.4, 0.8, 1.5 }
                    for _, d in ipairs(delays) do
                        char:AddGameTimer(d, false, function()
                            if slua.isValid(weapon) then
                                applySkinToWeaponRef(weapon)
                            end
                        end)
                ...
                if KillInfo and KillInfo.__inner_impl then
                    ...
                    KillInfo.__inner_impl.FileItem = function(self, DamageRecordData)
                        pcall(function()
                            ...
                            -- [FIX] 近战 ID 规范化，保证击杀计数 key 一致 (108001 → 108)
                            if weaponID and weaponID >= 108000 and weaponID <= 108999 then
                                weaponID = 108
                            end
                            local skinID = _G.get_skin_id and _G.get_skin_id(weaponID) or weaponID
        ...
        _G.RefreshKillCounterUI = function()
            pcall(function()
                ...
                -- [FIX] 近战 ID 规范化，保证击杀计数 key 一致
                if wID >= 108000 and wID <= 108999 then wID = 108 end
                ...
                -- [FIX] 只有真正装备了 Mod 皮肤 (>1000000) 才刷新 UI，防止 UI 闪烁
                local isModdedSkin = (sid and tonumber(sid) and tonumber(sid) > 1000000 and tonumber(sid) ~= wID)
                if not isModdedSkin then
