...
local function get_skin_id(currentGunId, maxIt)
    ...
    if currentGunId <= 0 and maxIt <= 0 then return 0 end
        ...
        function _G.equip_weapon_avatar(uCharacter)
            ...
            -- [FIX] 补充处理近战武器：GetAllInventoryWeaponList 通常不返回近战
            pcall(function()
                local meleeWpn = nil
                -- 尝试 prop slot 0 (近战专用槽)
                pcall(function()
                    if wm.GetInventoryWeaponByPropSlot then
                        meleeWpn = wm:GetInventoryWeaponByPropSlot(0)
                    end
                end)
                -- 尝试当前手持武器（若为近战）
                if not slua.isValid(meleeWpn) then
                    pcall(function()
                        if uCharacter.GetCurrentWeapon then
                            local cw = uCharacter:GetCurrentWeapon()
                            if slua.isValid(cw) then
                                local wid = 0
                                pcall(function() wid = cw:GetWeaponID() end)
                                if wid and (wid == 108 or (wid >= 108000 and wid <= 108999)) then
                                    meleeWpn = cw
                                end
                            end
                        end
                ...
                -- 尝试 WeaponManager 的 melee 接口
                if not slua.isValid(meleeWpn) then
                    pcall(function()
                        if wm.GetMeleeWeapon then meleeWpn = wm:GetMeleeWeapon() end
                    end)
                end
                if slua.isValid(meleeWpn) then
                    if applyWeaponSkinDirect(meleeWpn) then appliedAny = true end
                end
            end)
