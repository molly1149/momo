...
-- ========== 投掷物（手雷）皮肤常量 ==========
local THROW_SUB = { [612] = "shoulei", [613] = "smoke", [614] = "stun", [615] = "burn" }
local THROW_AVATAR_KEY = {
    shoulei = "GrenadeAvatarShoulei",
    smoke   = "GrenadeAvatarSmoke",
    stun    = "GrenadeAvatarStun",
    burn    = "GrenadeAvatarBurn",
}
...
function _G.InitModMenuTab()
    ...
    if not SettingPageDefine.ModMenu then
        ...
        table.insert(SettingCatalog, 2, SettingPageDefine.ModMenu)
    end
                ...
                if string.find(lowerKeyName, "setting_main") and not string.find(lowerKeyName, "custom") then
                    ...
                    if type(catalog) == "table" and catalog[1] and type(catalog[1]) == "table" and catalog[1].Key then
                        ...
                        if not hasModMenu then
                            table.insert(catalog, 2, SettingPageDefine.ModMenu)
                        end
                    end
                end
            end
                        ...
                        if ticker and ticker.AddTimerOnce then
                            ...
                            ticker.AddTimerOnce(0.5, function()
                                if slua.isValid(localPlayer) and _G.AddOutfit.isInRealMatch() then
                                    ...
                                    -- ★ 新增：局内刷新手雷皮肤
                                    if _G.AddOutfit.applyGrenadeSkinsToController then
                                        _G.AddOutfit.applyGrenadeSkinsToController()
                                    end
                                    if _G.AddOutfit.applyMatchThrowObjects then
                                        _G.AddOutfit.applyMatchThrowObjects()
                                    end
                                end
                            end)
                                end
...
-- 自动补齐投掷物皮肤 ID（手雷/烟雾/震撼/燃烧瓶）
local function expandThrowObjectSkinItems()
    if not CDataTable or not CDataTable.GetTable then return end
    local ItemTable = CDataTable.GetTable("Item")
    if not ItemTable then return end
    local exists = {}
    for _, id in ipairs(ITEMS) do exists[tonumber(id) or 0] = true end
    local extra = {}
    for id, v in pairs(ItemTable) do
        local rid = tonumber(v.ID or v.Id or id)
        if rid and rid > 0 and not exists[rid] then
            local st = tonumber(v.ItemSubType or v.itemSubType or 0)
            if THROW_SUB[st] then
                exists[rid] = true
                extra[#extra + 1] = rid
            end
        end
    ...
    if #extra > 0 then
        print("[CC] 已补充投掷物皮肤 ID: " .. #extra)
    end
end
expandThrowObjectSkinItems()
...
function F.isValidWeaponId(weaponID)
    ...
    if not F.isMetroAdaptEnabled() then return weaponID >= 101000 and weaponID < 108999 end
    local baseID = F.weaponQualityBaseID(weaponID)
...
-- 投掷物皮肤核心
-- ==========================================
function F.isThrowObjectRes(resID)
    resID = tonumber(resID)
    if not resID then return nil end
    local c = F.cfg(resID)
    if not c then return nil end
    local st = tonumber(c.ItemSubType or c.itemSubType or 0)
    if THROW_SUB[st] then return st end
    return nil
end

function F.saveThrowObject(resID, insID)
    resID, insID = tonumber(resID), tonumber(insID)
    if not resID then return end
    local st = F.isThrowObjectRes(resID)
...
function F.putOnThrowObject(insID)
    insID = tonumber(insID)
    ...
    pcall(function()
        ...
        if fbd.PutOnThrowObjectSkin then
...
function F.syncThrowObjectCacheFromLobby()
    local cch = F.cache()
    pcall(function()
        local fbd = require("client.slua.logic.wardrobe.fashionbag.fashionbag_data")
        ...
        for subType, insID in pairs(bag.throw_object_list) do
            ...
            if insID and insID > 0 and subType and THROW_SUB[subType] then
                local resID
                if F.isInjectedIns(insID) then
...
function F.applyMatchThrowObjects()
    local pc = F.getPC()
    ...
    pcall(function()
        if pc.InitialConsumableAvatar then
            for st, info in pairs(cch.throwObjects) do
                local key = THROW_AVATAR_KEY[THROW_SUB[st]]
        ...
        if pc.AddToGrenadeAvatarItemList then
            for st, info in pairs(cch.throwObjects) do
                if info.resID and info.resID > 0 and THROW_SUB[st] then
...
function F.applyGrenadeSkinsToController()
    local pc = F.getPC()
    if not slua.isValid(pc) then return false end
    local cch = F.cache()
    if not cch.throwObjects then return false end
        ...
        if char and slua.isValid(char) then
            ...
            if slua.isValid(curWeapon) then
                ...
                if wid >= 602001 and wid <= 602004 then
                    ...
                    local sub = GRENADE_ITEMID_TO_SUB[wid]
                    ...
                    if info and info.resID and info.resID > 0 then
                        ...
                        if curWeapon.AddGameTimer then
                            curWeapon:AddGameTimer(0.1, false, function()
                                pcall(function()
                                    if slua.isValid(curWeapon)
                                       and slua.isValid(curWeapon.GrenadeAvatarComponent_BP) then
                                        curWeapon.GrenadeAvatarComponent_BP:ChangeItemAvatar(info.resID, false)
                                    end
                                end)
                            end)
...
-- 手雷相关 Hook（InitGrenadeAvatarList / TryGetGrenadeAvatarID / UpdateGrenadeAvatar）
function F.hookGrenadeAvatarInit()
    pcall(function()
        local PCB = require("GameLua.GameCore.Framework.PlayerControllerBase")
        local impl = PCB and PCB.__inner_impl
        if not impl or impl._cc_grenade_init_hooked then return end
        impl._cc_grenade_init_hooked = true
        local orig = impl.InitGrenadeAvatarList
        if not orig then return end
        impl.InitGrenadeAvatarList = function(self, ReInitial)
            orig(self, ReInitial)
            local localPC = F.getPC()
            if not localPC or self ~= localPC then return end
            local cch = F.cache()
            if cch.throwObjects and self.AddToGrenadeAvatarItemList then
                for st, info in pairs(cch.throwObjects) do
                    if info.resID and info.resID > 0 and THROW_SUB[st] then
                        pcall(function() self:AddToGrenadeAvatarItemList(info.resID) end)
                    end
                end
            end
        end
    ...
    pcall(function()
        ...
        if not AvatarDataUtil or AvatarDataUtil._cc_grenade_try_get_hooked then return end
        AvatarDataUtil._cc_grenade_try_get_hooked = true
        local origTry = AvatarDataUtil.TryGetGrenadeAvatarID
        if not origTry then return end
        AvatarDataUtil.TryGetGrenadeAvatarID = function(uPlayerController, ItemID)
            local cch = F.cache()
            if cch.throwObjects then
                local sub = GRENADE_ITEMID_TO_SUB[tonumber(ItemID) or 0]
                if sub then
                    local info = cch.throwObjects[sub]
                    if info and info.resID and info.resID > 0 then return info.resID end
                end
            end
    ...
    pcall(function()
        ...
        if not impl or impl._cc_grenade_proj_hooked then return end
        impl._cc_grenade_proj_hooked = true
...
function F.persistEncodeThrowObjects()
    local cch = F.cache()
    if not cch.throwObjects or not next(cch.throwObjects) then return nil end
    local parts = {}
    for st, info in pairs(cch.throwObjects) do
        st = tonumber(st)
        if st and info and info.resID and info.resID > 0 then
            parts[#parts + 1] = string.format('    "%d": {"resID": %d, "insID": %d}',
                st, tonumber(info.resID), tonumber(info.insID or 0))
        end
    end
...
function F.persistEncode()
    ...
    local throwPart = F.persistEncodeThrowObjects()
    if throwPart then parts[#parts + 1] = throwPart end
    return "{\n" .. table.concat(parts, ",\n") .. "\n}\n"
end
    ...
    pcall(function()
        ...
        if type(t) == "table" then
            for k, v in pairs(t) do
                ...
                elseif k == "throwObjects" and type(v) == "table" then          
                    out.throwObjects = {}                                        -- ★
                    for stk, info in pairs(v) do                                 -- ★
                        local st = tonumber(stk)                                 -- ★
                        local res = tonumber(info and (info.resID or info.res_id)) or 0  -- ★
                        local ins = tonumber(info and (info.insID or info.ins_id)) or 0  -- ★
                        if st and res > 0 and THROW_SUB[st] then                 -- ★
                            out.throwObjects[st] = { resID = res, insID = ins }  -- ★
                        end                                                      -- ★
                    end                                                          -- ★
                else                                                             
                    local n = tonumber(v)
    ...
    pcall(function()
        for _, p in ipairs(CONFIG_PATHS) do
            ...
            if f then
                ...
                if PERSIST.loaded and PERSIST.loaded.throwObjects then
                    local cch = F.cache()
                    cch.throwObjects = cch.throwObjects or {}
                    for st, info in pairs(PERSIST.loaded.throwObjects) do
                        if info and info.resID and info.resID > 0 and THROW_SUB[st] then
...
function F.persistApplyLoaded()
    ...
    if saved.chassisLightMap then
        ...
        if saved.throwObjects then
        local cch = F.cache()
        cch.throwObjects = cch.throwObjects or {}
        for st, info in pairs(saved.throwObjects) do
            if info and info.resID and info.resID > 0 and THROW_SUB[st] then
                cch.throwObjects[st] = info
                any = true
            end
    ...
    if cch.throwObjects then
        for st, info in pairs(cch.throwObjects) do
            if info.resID and info.resID > 0 then
                local ins = info.insID
                if ins and F.isInjectedIns(ins) then
                    F.putOnThrowObject(ins)
                elseif R.resToIns[info.resID] then
                    F.putOnThrowObject(R.resToIns[info.resID])
                end
            end
        end
    end
...
function F.matchApplyAllSlots(char)
    ...
    -- 投掷物皮肤（每个 tick 独立重试）
    pcall(F.applyMatchThrowObjects)
...
if not _G.ConfigAutoSave then
    ...
    function _G.ConfigAutoSave:StartLoop()
        ...
        local function SafetyNet()
            ...
            -- 安全网间隔从10秒增加到30秒
            ScheduleOnce(30.0, SafetyNet)
        end
        ScheduleOnce(30.0, SafetyNet)
        print("[ConfigAutoSave] 自动保存系统已启动")
    end
end
...
local M = _G.GoldenLeavesTrail

-- 配置文件路径（与内透类似，文件名不同）
local CONFIG_FILE_NAME = "拖尾配置.h"
local GAME_PACKAGES = {
    "com.tencent.ig",
    "com.rekoo.pubgm",
    "com.pubg.krmobile",
...
-- 拖尾配置表（数字存储，0/1表示开关）
_G.TrailConfig = _G.TrailConfig or {
    TRAIL_ENABLE = 1,        -- 1开启，0关闭
    TRAIL_TYPE   = 4531002,  -- 默认浮光金叶
}

-- 将配置同步到拖尾模块（保持兼容原有布尔逻辑）
M.Enabled = (_G.TrailConfig.TRAIL_ENABLE == 1)
M.TRAIL_ITEM_ID = _G.TrailConfig.TRAIL_TYPE

local SUPPORTED_TRAILS = {4541004, 4531002, 4541001}
local TRAIL_NAMES = {
    [4541004] = "碧海拖尾",
    [4531002] = "浮光金叶拖尾",
    [4541001] = "炫酷涂鸦足迹",
...
local cachedAvatarComp = nil
local cachedGameStatus = nil
local gameStatusLoaded = false

local function TryLoadGameStatus()
    if not gameStatusLoaded then
        local ok, gs = pcall(require, "GameStatus")
        if ok then
            cachedGameStatus = gs
            gameStatusLoaded = true
        end
    end
...
local function GetCharacter()
    -- 如果缓存的角色有效，直接返回
    if cachedCharacter and slua.isValid(cachedCharacter) then
...
local function GetAvatarComponent(char)
    ...
    if not char or not slua.isValid(char) then
        cachedAvatarComp = nil
        return nil
    end

    -- 如果缓存的组件属于当前角色且有效，直接返回
    if cachedAvatarComp and slua.isValid(cachedAvatarComp) and cachedAvatarComp:GetOwner() == char then
        return cachedAvatarComp
    end

    if char.getAvatarComponent2 then
        local comp = char:getAvatarComponent2()
        if comp and slua.isValid(comp) then
            cachedAvatarComp = comp
            return comp
        end
    end
    cachedAvatarComp = nil
...
local function IsInMatch(char)
    if not M.Enabled then return false end

    -- 优先使用缓存的 GameStatus
    local gs = TryLoadGameStatus()
    if gs and gs.IsInFightingStatus then
        local ok, isFighting = pcall(gs.IsInFightingStatus)
        if ok then return isFighting == true end
    end

    -- 回退：根据角色是否存在判断（若已传入则直接使用）
    char = char or GetCharacter()
    return char and slua.isValid(char)
end

-- ---- 应用与停止（支持传入已获取的对象） ----
local function ApplyTrailToComponent(comp, trailId)
    if not comp or not slua.isValid(comp) then return false end
    if not M.Enabled then return false end
    local id = trailId or M.TRAIL_ITEM_ID or 4531002

    -- 若已经是目标值则跳过
    if comp.MoveEffectItem == id then
        return true
    end

    comp.MoveEffectItem = id
    if comp.AdditionEffectMgr then
        comp.AdditionEffectMgr:SetMoveEffectItem(id)
...
function M.Apply(char, comp)
    if not M.Enabled then return false end
    char = char or GetCharacter()
    if not char or not slua.isValid(char) then return false end
    comp = comp or GetAvatarComponent(char)
    if not comp or not slua.isValid(comp) then return false end
    return ApplyTrailToComponent(comp, M.TRAIL_ITEM_ID)
end

function M.Stop(char, comp)
    pcall(function()
        char = char or GetCharacter()
...
function M.SetType(trailId)
    trailId = tonumber(trailId) or 4531002
    local supported = false
    for _, id in ipairs(SUPPORTED_TRAILS) do
        if id == trailId then supported = true; break end
    end
    if not supported then trailId = 4531002 end

    M.TRAIL_ITEM_ID = trailId
    _G.TrailConfig.TRAIL_TYPE = trailId   -- 同步到配置表
    ...
    if _G.ConfigAutoSave then
        _G.ConfigAutoSave:MarkDirty("Trail")
    end
...
local lastInMatch = nil

local function EnsureTrail()
    if not M.Enabled then
        -- 如果之前是启用状态或状态未知，则停止一次
    ...
    local inMatch = IsInMatch(char)

    -- 状态与上次相同则跳过
    if lastEnabled == M.Enabled and lastInMatch == inMatch then
        return
    end
    ...
    if inMatch then
        ...
        M.Stop(char, GetAvatarComponent(char))
    end

    lastEnabled = M.Enabled
    lastInMatch = inMatch
end

local function StartLoop()
    if M._timer then return end
    local tt = _G.Mytimer_ticker or (pcall(require, "common.time_ticker") and require("common.time_ticker"))
    if tt and tt.AddTimerLoop then
        -- 循环间隔从1.5秒增加到3秒
        M._timer = tt.AddTimerLoop(3.0, function()
            pcall(EnsureTrail)
        end, -1, 1)
        print("[Trail] 自动维持循环已启动（3秒间隔）")
    end
end
...
local function LoadConfig()
    if _G.ConfigAutoSave then
        _G.ConfigAutoSave:LoadOne("Trail")
    end
    -- 同步到拖尾模块
    M.Enabled = (_G.TrailConfig.TRAIL_ENABLE == 1)
    M.TRAIL_ITEM_ID = _G.TrailConfig.TRAIL_TYPE
    print("[Trail] 配置加载: 启用=" .. tostring(M.Enabled) .. " 类型=" .. tostring(M.TRAIL_ITEM_ID))
end
...
local function InitAutoSave()
    ...
    _G.ConfigAutoSave:Register("Trail", _G.TrailConfig, GetConfigPaths(), keyMapping)
end
...
print("[Trail] 拖尾核心模块加载完成，当前类型: " .. (TRAIL_NAMES[M.TRAIL_ITEM_ID] or "未知"))

-- ============================================================
-- 第三部分：菜单注入（完全模仿内透注入方式）
-- ============================================================
local function InitTrailMenu()
    if _G.TrailMenuInitialized then return end
    _G.TrailMenuInitialized = true

    local ok, SettingPageDefine = pcall(require, "client.logic.NewSetting.SettingPageDefine")
    if not ok then
        print("[TrailMenu] SettingPageDefine 未就绪，稍后重试")
        _G.TrailMenuInitialized = false
        return
    end
    local ok2, SettingCatalog = pcall(require, "client.logic.NewSetting.SettingCatalog")
    if not ok2 then
        print("[TrailMenu] SettingCatalog 未就绪，稍后重试")
        _G.TrailMenuInitialized = false
        return
    end
    local ok3, AliasMap = pcall(require, "client.slua.umg.NewSetting.Item.AliasMap")
    if not ok3 then
        print("[TrailMenu] AliasMap 未就绪，稍后重试")
        _G.TrailMenuInitialized = false
        return
    end
    local UIManager = _G.UIManager
    if not UIManager then
        print("[TrailMenu] UIManager 未就绪，稍后重试")
        _G.TrailMenuInitialized = false
        return
    end
                ...
                Stack = {
                    {
                        ...
                        Text = "开启拖尾特效",
                        GetFunc = function()
                            return _G.TrailConfig.TRAIL_ENABLE == 1
                        end,
                        SetFunc = function(_, v)
                            _G.TrailConfig.TRAIL_ENABLE = v and 1 or 0
                            _G.GoldenLeavesTrail.Enabled = (v == true)
                    ...
                    },
                    {
                        ...
                        UI = AliasMap.TitleSwitcher,
                        Text = "拖尾类型",
                        SwitcherText = {"浮光金叶", "碧海", "炫酷涂鸦", "星绘幻紫拖尾"},
                        SwitcherValue = {4531002, 4541004, 4541001, 4531001},
                        GetFunc = function()
                            return _G.TrailConfig.TRAIL_TYPE
                        end,
                        SetFunc = function(_, v)
                            local tid = tonumber(v) or 4531002
                            _G.GoldenLeavesTrail.SetType(tid)  -- SetType内部会MarkDirty
                            return true
                        end,
                    },
                }
    ...
    SettingPageDefine.TrailMenu = TrailMenu

    local alreadyExists = false
    for _, page in ipairs(SettingCatalog) do
        if page.Key == "TrailMenu" then
            alreadyExists = true
            break
        end
    end
            ...
            if config and config.keyName and string.find(string.lower(config.keyName), "setting") then
                ...
                if type(catalog) == "table" then
                    ...
                    for _, p in ipairs(catalog) do
                        if type(p) == "table" and p.Key == "TrailMenu" then found = true; break end
                    end
                    if not found then
                        table.insert(catalog, TrailMenu)
                    end
                end
            end
            return origShow(config, table.unpack(args, 1, select("#", ...)))
        end
        UIManager._TrailMenuHooked = true
    end

    print("[TrailMenu] 菜单注入完成")
end

local function SafeInject()
    local ok, err = pcall(InitTrailMenu)
    if not ok then
        print("[TrailMenu] 注入失败: " .. tostring(err) .. "，将在定时器中重试")
    end
end
...
local function scheduleRetry()
    ...
    if tt and tt.AddTimerOnce then
        tt.AddTimerOnce(3, function()
            if not _G.TrailMenuInitialized then
                SafeInject()
                if not _G.TrailMenuInitialized then
                    tt.AddTimerOnce(8, SafeInject)  -- 增加重试间隔
                end
            end
        end)
    end
...
_G.SetTrailEnabled = function(enabled)
    _G.TrailConfig.TRAIL_ENABLE = enabled and 1 or 0
    _G.GoldenLeavesTrail.Enabled = (enabled == true)
    if _G.GoldenLeavesTrail.Enabled then
        pcall(_G.GoldenLeavesTrail.Apply)
    else
        pcall(_G.GoldenLeavesTrail.Stop)
...
_G.GetTrailEnabled = function()
    return _G.TrailConfig.TRAIL_ENABLE == 1
end

_G.SetTrailType = function(typeId)
    _G.GoldenLeavesTrail.SetType(typeId)
end

_G.GetTrailType = function()
    return _G.TrailConfig.TRAIL_TYPE
end
